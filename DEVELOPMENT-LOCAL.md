...
- **Governance & accepted RFCs**: see `docs/rfcs/`. **RFC-0001** (`docs/rfcs/0001-modules-tiers-governance-and-automation-placement.md`, Accepted 2026-05-18) commits the canonical-module rule, the five reserved canonical codes (`mrp` / `wms` / `crm` / `crp` / `automation`; `brewery` is the canonical tier-6 vertical-configuration example), governance for canonical-code allocation, and the horizontal-platform-services consumption contract (modules consume — never reimplement — auth / tenancy / ACL / billing / AI / observability / i18n / UI / secrets / integrations / HTTP / DB). Read RFC-0001 before any feature work that touches module shape, AI-tool registration, billing or tier limits, or any cross-cutting platform concern.
