...
Checks (per README):
  ...
  3. Brand callout present (a `> [!NOTE]` block referencing
     docs/RENAME-DILIGENCE.md on first mention of "Umbraculum").
...
def check_brand_callout(text: str, result: CheckResult) -> None:
    ...
    if "RENAME-DILIGENCE.md" not in block_text:
        result.failures.append(
            "Brand callout does not link to docs/RENAME-DILIGENCE.md."
        )
    if "Umbraculum" not in block_text:
        result.failures.append(
            "Brand callout does not mention `Umbraculum`."
        )
