...
- **Depends on**: `services/api` (HTTP backend); `@brewery/contracts` (typed responses); `@umbraculum/ui` + `@brewery/recipes-ui` (UI primitives + domain UI); `@brewery/i18n` + `@brewery/i18n-react` (localization); `@umbraculum/navigation` (route ID system shared with native); `@umbraculum/media` (assets); `@brewery/beerjson` (recipe parsing/normalization); `@brewery/core` (math primitives, gravity/SG conversions).
