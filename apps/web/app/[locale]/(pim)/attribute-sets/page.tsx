...
import { listAttributeSets } from "@umbraculum/api-client/pim";
...
/**
 * PIM attribute sets list — Week 1 audit shape.
 *
 * URL: `/en/attribute-sets`. Previously `/en/pim/attribute-sets` per
 * the URL-axis layout corrected by the audit ([`docs/design/
 * web-route-group-audit.md`](../../../../../../docs/design/web-route-group-audit.md)).
 * `attribute-sets` is one of three canonical static sub-segments the
 * PIM module owns via `registerWebModule({ ownedUrlSegments })`.
 */
export default function PimAttributeSetsPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const data = await listAttributeSets(client);
      setSets(data.items);
    } catch (err) {
      setError(String(err));
      setSets([]);
            ...
            {sets.map((s) => (
              <li key={s.id} className="brew-recipe-list-row">
                <YStack gap="$1">
                  ...
                  <Link href={`/attribute-sets/${s.id}`}>{tSets("openDetail")}</Link>
                </YStack>
              </li>
            ))}
          </ul>
