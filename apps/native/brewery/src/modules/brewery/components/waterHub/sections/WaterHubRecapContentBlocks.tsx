...
export function WaterHubRecapStatsBlock(props: { model: NativeWaterHubScreenModel }) {
  const { t, tUnits, summary, displayStreams, fmt, displayAlkalinityPpmCaCO3 } = props.model;

  if (!summary || !displayStreams) return null;
...
export function WaterHubRecapAdditionsBlock(props: { model: NativeWaterHubScreenModel }) {
  const { t, displayStreams } = props.model;

  if (!displayStreams) return null;
