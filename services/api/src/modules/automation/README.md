...
**Out of scope here, by design (per [`docs/design/canonical-automation-module-surface.md`](../../../../../docs/design/canonical-automation-module-surface.md) §11 Non-goals):** vessel **scheduling**, **utilization**, **capacity planning**, **booking**, "next planned use", or any forecasting view. Those views belong to the future `crp` canonical module (one of the five reserved codes from RFC-0001 Decision B alongside `mrp`, `wms`, `crm`, `automation`). When `crp` ships, it will read the same `vesselId` through `@umbraculum/equipment-contracts` (extracted from the brewery-internal `EquipmentProfile` per [`docs/design/canonical-automation-module-surface.md`](../../../../../docs/design/canonical-automation-module-surface.md) §4) and render the vessel-as-planning-resource view on its own surface.
