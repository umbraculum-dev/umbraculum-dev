...
describe("ai (chat + settings + gating)", () => {
  ...
  it("module-owned AI tools from automation and PIM are available at boot", async () => {
    mockCreate.mockResolvedValueOnce({
      id: "msg_test_module_tools_1",
      content: [
        { type: "tool_use", id: "toolu_automation", name: "automation.listVessels", input: {} },
        { type: "tool_use", id: "toolu_pim", name: "pim.listCategories", input: {} },
      ],
      stop_reason: "tool_use",
      usage: { input_tokens: 20, output_tokens: 10 },
    });
    mockCreate.mockResolvedValueOnce({
      id: "msg_test_module_tools_2",
      content: [{ type: "text", text: "I checked the installed module tools." }],
      stop_reason: "end_turn",
      usage: { input_tokens: 30, output_tokens: 15 },
    });
    ...
    const res = await app.inject({
      ...
      url: "/ai/chat",
      headers: { cookie: adminCookie, "content-type": "application/json" },
      payload: { message: "check vessels and categories" },
    });
    expect(res.statusCode).toBe(200);
    expect(res.payload).toContain("automation.listVessels");
    expect(res.payload).toContain("pim.listCategories");
    expect(res.payload).toContain("event: tool_result");
    expect(res.payload).toContain("event: complete");
  });
