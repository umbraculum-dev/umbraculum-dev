/**
 * inventory.test.ts — Phase 4b-2 L2 coverage for `services/api/src/routes/inventory.ts`.
 *
...
describe("inventory routes (Phase 4b-2)", () => {
  ...
  describe("cross-workspace isolation (Phase 4b-2 ↔ 4b-1 parity)", () => {
    // The cross-workspace pattern matches Phase 4b-1 (brew-sessions): the
    // route handlers' Prisma queries scope by `workspaceId`, so persona B
    // probing A's `:id` gets 404 (NotFoundError → HTTP 404 in
    // `services/api/src/errors.ts`). These tests pin that scoping so a
    // future regression that drops the `workspaceId` filter (e.g. on a
    // refactor) fails loudly here rather than silently leaking inventory.
