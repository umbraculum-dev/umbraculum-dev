...
**Audience:** project maintainers and future trademark counsel
**Document role:** private legal/history artifact preserving the brand-diligence record; intentionally excluded from public docs and docs-site navigation.

---

## 1. Resolution at a glance

The project's historical platform-name placeholder (per [PLATFORM-ARCHITECTURE.md §10](../docs/PLATFORM-ARCHITECTURE.md)) resolves to the following set of values across surfaces:
...
| Mascot | **Umbi** — character expression of the brand; canonical asset at [`docs/media/umbi.png`](../docs/media/umbi.png). The diminutive form mirrors the wordmark's own diminutive Latin morphology (*umbraculum* = "small parasol"; *umbi* = the mascot's first-name-style short form). |
...
- **Name**: Umbi
- **Canonical asset**: [`docs/media/umbi.png`](../docs/media/umbi.png) (1254×1254, PNG)
...
This resolution unblocked the rename sub-plan's `find_placeholders_grep` → substitution → confirmation pipeline against the brewery-app repo and the openplc sister-repo. The `@brewery/*` package scopes — originally scoped per the umbrella plan §10 exclusions as out-of-scope of the brand rename — were migrated separately as sub-plan #9 (now closed); see below.

**Sub-plan #9 status — CLOSED 2026-05-19.** All 14 slots shipped: see [`docs/design/brewery-scope-migration-plan.md`](../docs/design/brewery-scope-migration-plan.md) §0 banner for the closure record + slot-by-slot recaps, and [`docs/design/brewery-scope-migration-per-package-handoff.md`](../docs/design/brewery-scope-migration-per-package-handoff.md) for the per-slot execution log. Outcome: horizontal packages now live under `@umbraculum/*`; brewery-vertical packages re-scoped under `@umbraculum/brewery-*` per the §1.3 TRAP-avoidance discipline; the four application workspaces (`api`, `web`, `native`, `web-e2e`) flipped on slot 14 (closing slot). Lessons codified into the project's plugin-pack as the `package-scope-migration-preflight` skill + the `ci-parity-local-reproduction` skill + rule 72 (local-vs-CI divergence mechanisms).
