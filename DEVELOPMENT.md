...
- **CI parity over local-only signals.** Use **T1** slice verification while iterating (`npm run verify:from-diff` or a named `verify:*` script — see [`docs/VERIFICATION-TIERS.md`](docs/VERIFICATION-TIERS.md)). **Before pushing**, run **T2**: `npm run verify:pre-push` or `./scripts/ci-parity-check.sh run` (mounts your **working tree** via `--ci`, including uncommitted edits) plus the **`api-integration-tests-pre-push`** skill when API paths changed. Use `… run --archive` or `--sha <ref>` only when replaying a committed ref. See [`docs/CI-PARITY.md`](docs/CI-PARITY.md) §Snapshot modes and the `ci-parity-local-reproduction` skill.
