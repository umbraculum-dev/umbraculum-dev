...
- **CI parity over local-only signals.** Use **T1** slice verification while iterating (`npm run verify:from-diff` or a named `verify:*` script — see [`docs/VERIFICATION-TIERS.md`](docs/VERIFICATION-TIERS.md)). **Before pushing**, run **T2**: commit first, then `npm run verify:pre-push` (uses **`--archive`** on a clean tree — cold `npm ci` on committed HEAD) plus the **`api-integration-tests-pre-push`** skill when API paths changed. **Agents** run T2 themselves via **`./scripts/ci-parity-check.sh`** — not bare `npx @umbraculum/ci-parity` (Cursor AppImage `npx` can ENOENT). Use `./scripts/ci-parity-check.sh run` (`--ci`) only for WIP iteration, not as the push gate. See [`docs/CI-PARITY.md`](docs/CI-PARITY.md) §Snapshot modes and the `ci-parity-local-reproduction` skill.
...
- **`ci-parity-local-reproduction`** (node-react) — T2 static analysis; default pre-push uses `./scripts/ci-parity-check.sh run` (`--ci`, working tree). Use `--archive` / `--sha` via the skill when replaying a committed ref.
