...
- **npm lockfiles (monorepo policy — Composer analogy).** `package.json` ≈ `composer.json` (constraints). Each committed `package-lock.json` ≈ `composer.lock` (exact resolved tree). **`npm ci`** ≈ **`composer install`** (install from lock; do not rewrite it). **`npm install`** / **`npm-lock-update.sh`** ≈ **`composer update`** (re-resolve and rewrite lock). If a committed lockfile changes locally, dependencies changed and tests ran against that tree — **always commit the lockfile together with the matching `package.json`** before push. **`git restore` on a lock alone is not a revert** — full revert is **`npm run lock:revert`** (restore lock from git **and** reinstall `node_modules` from it).
  - **Committed lockfiles (always versioned):** root `package-lock.json` (all root workspaces including `@umbraculum/api` deps), `apps/web/package-lock.json`, `apps/web/e2e/package-lock.json`, `packages/platform/test-mcp/package-lock.json`, `docs-site/vendor/brochure/package-lock.json`.
  - **Forbidden (never commit):** `services/api/package-lock.json` — ephemeral artifact if someone runs `npm install` with `cwd=services/api`; API versions live in **root** lock only. [`services/api/.npmrc`](services/api/.npmrc) sets `package-lock=false`; api compose deletes stray file after install; [`scripts/check-monorepo-lockfiles.sh`](scripts/check-monorepo-lockfiles.sh) fails if it appears. **`rm -f services/api/package-lock.json`** — never `git add -f`.
  - **Agent commands:** `npm run lock:install` / `lock:install:root` / `lock:install:web` (composer install); `npm run lock:update:root` / `lock:update:web` (composer update); `npm run lock:revert` (full revert). Regen in **`node:20-slim`** or compose **`web`/`api`** — not host npm for commits. **`npm run check:lockfiles`** before commit when lockfiles change; **`verify:pre-push`** runs it in T2 validate. Skill **`monorepo-lockfile-gate`**; rule **`80-monorepo-lockfile-agent-gate`**. Web compose boot uses **`npm ci`** (not `npm install`) so bind-mounted locks are not rewritten on every start.
- **Published SDK dogfood (contracts + api-client + MIT SDK batch).** Consumer manifests (`apps/web`, `services/api`, `packages/platform/rendering`) pin published `@umbraculum/*` at registry semver (`^0.0.1` contracts/api-client; `^0.1.0` MIT batch per [RFC-0012](docs/rfcs/0012-package-tier-clarity.md)); npm workspaces symlink in-tree sources for active co-dev. Native brewery uses `file:` for `@umbraculum/module-sdk` (monorepo co-dev). Publisher packages (`packages/platform/api-client`, `packages/sdk/module-sdk`) keep `file:` co-dev links. After each npm semver bump, run [`scripts/dogfood-npm-smoke.sh`](scripts/dogfood-npm-smoke.sh). Policy: [`docs/design/npm-sdk-monorepo-dogfood.md`](docs/design/npm-sdk-monorepo-dogfood.md).
...
- **Brochure site (Cloudflare Workers):** [`umbraculum-brochure`](https://github.com/umbraculum-dev/umbraculum-brochure) builds static `umbraculum.dev` output; deploy per [`docs/design/public-alpha-cloudflare-pages-runbook.md`](docs/design/public-alpha-cloudflare-pages-runbook.md). Local preview: clone sibling repo and `npm run preview` (port 4321).
- **Brochure local dev:** not in `docker compose` — use [`umbraculum-brochure`](https://github.com/umbraculum-dev/umbraculum-brochure) at `../umbraculum-brochure`. Product nginx `:18080` does **not** serve the brochure.
