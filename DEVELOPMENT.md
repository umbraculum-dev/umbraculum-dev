...
- **CI parity over local-only signals.** Use **T1** slice verification while iterating (`npm run verify:from-diff` or a named `verify:*` script — see [`docs/VERIFICATION-TIERS.md`](docs/VERIFICATION-TIERS.md)). **Before pushing**, run **T2-PR**: commit first, then `npm run verify:pre-push` (path-aware parallel ci-parity on committed HEAD + auto API vitest when triggered). Use **`npm run verify:pre-push:release`** for manifest/SDK tag prep. **Agents** run T2 themselves via **`./scripts/ci-parity-check.sh`** — not bare `npx @umbraculum/ci-parity` (Cursor AppImage `npx` can ENOENT). Use `./scripts/ci-parity-check.sh run` (`--ci`) only for WIP iteration, not as the push gate. See [`docs/CI-PARITY.md`](docs/CI-PARITY.md) § T2-PR vs T2-release and the `path-aware-pre-push` skill.
