...
- **npm lockfiles (monorepo policy).** **`package-lock.json` at the repo root** is the single source of truth for every workspace listed in root `package.json` `workspaces`, including **`@umbraculum/api`** (`services/api`). **Do not commit** `services/api/package-lock.json` — it is not a second “tied version” surface; it is an **ephemeral artifact** created when `npm install` runs with `cwd=services/api` (the api compose service does this on every start). That stray lockfile diverges from root, pollutes `git status`, and does **not** govern CI (`npm ci` at repo root). **Intentional extra lockfiles** (documented exceptions): `apps/web/package-lock.json` (isolated web Docker install + `legacy-peer-deps`), `apps/web/e2e/package-lock.json`, `packages/platform/test-mcp/package-lock.json`, `docs-site/vendor/brochure/package-lock.json`. Prevention: [`services/api/.npmrc`](services/api/.npmrc) sets `package-lock=false`; compose removes any stray file after install; CI runs [`scripts/check-monorepo-lockfiles.sh`](scripts/check-monorepo-lockfiles.sh) and [`scripts/check-root-lockfile-sync.sh`](scripts/check-root-lockfile-sync.sh) (`npm run check:lockfiles`). **`verify:pre-push`** runs `check:lockfiles` on every T2 validate. **Agents** (not the operator) regen root lockfile in **`node:20-slim`** and run the gate before commit — see [`AGENTS.md`](AGENTS.md) § Monorepo lockfile gate and toolset rule **`80-monorepo-lockfile-agent-gate`** / skill **`monorepo-lockfile-gate`**. **After editing root `package-lock.json`**, regenerate in **`node:20-slim`** (not host npm) and confirm `npm ci` succeeds there — a lockfile that only satisfies host `npm install` will fail GHA `npm ci` (e.g. `@swc/helpers` peer drift). If you see the forbidden file: `rm -f services/api/package-lock.json` — do not `git add` it.
