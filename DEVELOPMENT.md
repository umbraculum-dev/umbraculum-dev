...
- **CI parity over local-only signals.** Local lint/typecheck can lie via documented divergence mechanisms (hoisted vs container `node_modules`, nested-workspace install gaps, gitignored cross-refs). **Before pushing**, run `./scripts/ci-parity-check.sh` from the repo root (reproduces docs-readmes, typecheck, and lint against a clean `git archive HEAD` snapshot). See [`docs/TYPING.md`](docs/TYPING.md) §"Before pushing — CI parity" and the `ci-parity-local-reproduction` skill.
