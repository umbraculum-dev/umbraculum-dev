...
Local-model integration for the cursor-rules subagent system.

This document covers how to drive [Ollama](https://ollama.com/) (or any OpenAI-compatible local endpoint) from the same `.cursor/agents/*.md` subagents that ship with this package.
...
This pattern reuses the same `.cursor/agents/*.md` files this package ships, only swapping the `model:` field. Cursor manages context isolation, parallelism, and tool access for you.
...
4. Drop a sibling subagent file for one of the shipped agents. Naming convention: `<name>.local.md`. The package sync overwrites by name only, so `*.local.md` files are preserved.
   ...
   You are a skeptical validator running on a local model. Same instructions as `.cursor/agents/verifier.md`:

   1. Read `DEVELOPMENT.md` and `DEVELOPMENT-LOCAL.md` first.
   2. For URL/exception fixes, follow `.cursor/skills/curl-exception-verification.md` exactly.
   3. For PHPUnit claims, follow `.cursor/skills/phpunit-unit-runbook.md` exactly.
...
- [.cursor/rules/14-subagent-contract.mdc](.cursor/rules/14-subagent-contract.mdc)
- [.cursor/agents/](.cursor/agents/) — shipped subagents (Pattern A starting points)
- [Cursor subagents docs](https://cursor.com/docs/subagents)
