...
- The module SDK's public surface ([`../../packages/module-sdk/README.md`](../../packages/module-sdk/README.md)).
...
When a future RFC lands (likely candidates: `0011-billing-model` covering `WorkspaceBillingAddon` + Stripe subscription-item flow + RevenueCat consumables; or an eventual successor RFC promoting the route-group β disciplines from plugin-pack rule [46-web-route-shape.mdc](https://github.com/umbraculum-dev/umbraculum-toolset) to RFC-level commitment if third-party modules contest them), it is added here first; the change is not done until this index reflects it. The brewery Postgres schema split from `public.*` to `platform.*` + `brewery.*` shipped under [RFC-0010](0010-platform-brewery-postgres-schema-split.md) (2026-05-28).
