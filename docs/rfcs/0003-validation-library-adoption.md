...
**Brand resolution:** the project's brand was resolved on 2026-05-18 to **Umbraculum**, npm scope `@umbraculum`. See [`docs/RENAME-DILIGENCE.md`](../RENAME-DILIGENCE.md) and [RFC-0002](0002-canonical-module-physical-layout.md).
...
The deferred cluster (§10) covers OpenAPI generation, the Zod v5 tracker, and the public plugin-pack distribution story — all explicitly out-of-scope of this RFC because they are gated on the public-flip H1 2027 milestone.
...
export interface RegisterModuleOptions {
  ...
  prismaSchema?: string;
  aiTools?: AiToolRegistration<ValidatedSchema<unknown>, ValidatedSchema<unknown>>[];
...
|---|---|---|---|
| **PR 1** | `packages/contracts/` — 8 parsers + 38 unit tests + ~25 consumer call sites in `apps/web` and `apps/native` | ~3 days | `.cursor/rules/22-typescript-contracts-runtime-validation.mdc` rewritten + new lint rule (`no-restricted-syntax` forbidding hand-rolled drift in `packages/*-contracts/`) |
| **PR 2** | `packages/automation-contracts/` — `mailbox-data.ts` (168→~55 lines) + `mailbox.ts` types collapse to `z.infer` + 28 mailbox tests | ~2 days | Skill: `.cursor/skills/scaffold-contracts-package/` |
| **PR 3** | `services/api/src/routes/` — 28 routes + `fastify-type-provider-zod` adoption + ~30 route-local `assert*` helpers + `apps/web` + `apps/native` client updates (no bridge layer) + ~80–100 integration tests | ~5 days | New rule: `.cursor/rules/<NN>-fastify-route-schema-validation.mdc` |
...
|---|---|
| PR 1 | Rewrite `.cursor/rules/22-typescript-contracts-runtime-validation.mdc` (was: "do NOT introduce Zod"; becomes: "use Zod v4 schemas, here is the canonical pattern, refer to `docs/CONTRACTS-VALIDATION-STRATEGY.md` v2.0") + new lint rule |
| PR 2 | New skill `.cursor/skills/scaffold-contracts-package/` for new `packages/<code>-contracts/` |
| PR 3 | New rule `.cursor/rules/<NN>-fastify-route-schema-validation.mdc` + new skill `.cursor/skills/scaffold-fastify-route-with-schema/` |
| PR 4 | Documentation in `@umbraculum/module-sdk/README.md` showing the library-agnostic pattern |
| Foundation update (Phase 4) | `.cursor/rules/02-foundation-hardening.mdc` (cross-slice anchor); `.cursor/rules/26-typescript-strict-flags.mdc`; `.cursor/rules/45-tsjs-module-readme-standard.mdc`; subagent `.cursor/agents/validation-baseline-verifier.md`; subagents `types-baseline-verifier.md` + `module-readme-checker.md` (originally proposed in §8.3 and §8.5 of the foundation-hardening doc) |
...
The 30-day public-comment period in [`docs/LICENSING.md`](../LICENSING.md) §10 applies post-public-flip (working assumption H1 2027). Until then: lead alone, mirrors the [RFC-0002](0002-canonical-module-physical-layout.md) sign-off pattern.
