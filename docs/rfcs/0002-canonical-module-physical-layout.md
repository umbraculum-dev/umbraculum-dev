...
- `registerModule(options)` — server-side registration (routes, prisma schema name, aiTools, tierLimits, addonCodes) per [`docs/PLATFORM-ARCHITECTURE.md`](../PLATFORM-ARCHITECTURE.md) §4.4 sketch.
...
## 6. Decision D — Brewery-as-first-vertical sequencing: wait for 2nd canonical module (commit)

**Brewery does NOT pre-migrate to β before the second canonical module ships.**
...
**Working assumption:** H1 2027 per [`docs/ROADMAP.md`](../ROADMAP.md) and [`docs/PLATFORM-ARCHITECTURE.md`](../PLATFORM-ARCHITECTURE.md) §5.2. If the second module ships earlier, the migration moves with it; Decision D is about sequencing principle, not a fixed calendar date.
