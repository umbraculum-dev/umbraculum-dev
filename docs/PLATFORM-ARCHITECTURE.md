...
**Related as-built docs (not platform vision).** Cross-platform (web + native + Ubuntu Touch webapp shell) boundaries: [`docs/CROSS-PLATFORM-BOUNDARIES.md`](CROSS-PLATFORM-BOUNDARIES.md), [`docs/design/ubuntu-touch-shell-strategy.md`](design/ubuntu-touch-shell-strategy.md). Brewery reference vertical product rules: [`docs/modules/verticals/brewery/IMPLEMENTATION-LOG.md`](modules/verticals/brewery/IMPLEMENTATION-LOG.md). Use **this document** for platform-level direction, public positioning, the module model, the AI consultant blueprint, and the monetization path.
...
- **Current state (2026-05-27).** The monorepo lives at **`github.com/umbraculum-dev/umbraculum-dev`** under the **`umbraculum-dev` GitHub org**, visibility **private**. The sister-repo **`github.com/umbraculum-dev/umbraculum-toolset`** (Cursor plugin pack source) is also org-hosted and private — no ownership transfer was needed for toolset. **Stage 0 org transfer is complete:** git history was rewritten via orphan `main` before transfer; **commits and tags were preserved** on the org repo; GitHub Actions workflows carried over; org push + **`docs-readmes`** workflow verified green (commit `6a1ea92`). Remaining before public flip: Stage 1 hygiene scan ([`public-alpha-preflip-hygiene-checklist.md`](design/public-alpha-preflip-hygiene-checklist.md)), Cloudflare deploy of docs + brochure, and the atomic visibility flip paired with `umbraculum-toolset` + marketplace submission. **MIT npm SDK publish batch ✅ 2026-05-29** (checklist item 6 — pre-completed). **In tree (2026-05-27):** URL canonicalization, docs-site lunr search + pre-flip `noindex`, brochure + `/support/` scaffold, DocSearch apply draft.
