...
- reuse one billing engine, one auth stack, one AI layer, and one cross-platform shell across every module and vertical,
...
**npm publication readiness (2026-05-27):** Source-complete for third-party module work **inside this monorepo** (`module-sdk`, `ai-tool-sdk`, `i18n-keys` landed; `dist/` committed; tests green). **Not registry-published yet** — external module repos cannot `npm install @umbraculum/module-sdk` until the July 2026 α cutover batch. Authoritative status table and publish checklist: [`docs/LICENSING.md`](LICENSING.md) §6.2.1. Roadmap tracking: [`docs/ROADMAP.md`](ROADMAP.md) (priority **6** + Late H1 tranche).
...
- **Current state (2026-05-27).** The monorepo lives at **`github.com/umbraculum-dev/umbraculum-dev`** under the **`umbraculum-dev` GitHub org**, visibility **private**. The sister-repo **`github.com/umbraculum-dev/umbraculum-toolset`** (Cursor plugin pack source) is also org-hosted and private — no ownership transfer was needed for toolset. **Stage 0 org transfer is complete:** git history was rewritten via orphan `main` before transfer; **commits and tags were preserved** on the org repo; GitHub Actions workflows carried over; org push + **`docs-readmes`** workflow verified green (commit `6a1ea92`). Remaining before public flip: Stage 1 hygiene scan ([`public-alpha-preflip-hygiene-checklist.md`](design/public-alpha-preflip-hygiene-checklist.md)), Cloudflare deploy of docs + brochure, **MIT npm SDK publish batch** (checklist item 6 below), and the atomic visibility flip paired with `umbraculum-toolset` + marketplace submission. **In tree (2026-05-27):** URL canonicalization, docs-site lunr search + pre-flip `noindex`, brochure + `/support/` scaffold, DocSearch apply draft.
- **Decision.** Target a **July 2026 public alpha** rather than the original H1 2027 public-flip horizon. The remaining prerequisites are now the pre-flip hygiene checklist, docs-site/public-docs readiness, brochure-site/DNS readiness, and the umbraculum-toolset marketplace path; a complete second vertical is no longer a blocker for public alpha. (The package-name migration prerequisite was retired by sub-plan #9, closed 2026-05-19.) At the flip, **seed a fresh public repository from this one under the `umbraculum-dev` GitHub org** rather than renaming this repository in place. The original repository remains as the private development history and is archived once the public seed catches up.
...
- **Pre-flip checklist** (kept here so the conditions are explicit, not folkloric):
  ...
  6. **MIT SDK npm publish batch** — publish `@umbraculum/ai-tool-sdk`, `@umbraculum/i18n-keys`, `@umbraculum/module-sdk`, and the canonical `@umbraculum/<code>-contracts` packages to the public npm registry under the `@umbraculum` scope (remove `private: true`, add MIT `license` metadata, replace inter-package `file:` deps with semver registry deps, publish leaves before `module-sdk`). **Not required for repo visibility flip** but **required before third-party module authors can depend on registry installs**; in-repo packages are source-ready per [`docs/LICENSING.md`](LICENSING.md) §6.2.1. `@umbraculum/api-client` remains deferred (contracts coupling — see that table).
...
- **Marketplace-submission and public-alpha closure criterion.** The four umbraculum-toolset Cursor plugins (`umbraculum-toolset-common`, `umbraculum-node-react-cursor-assistant`, `umbraculum-platform-tsjs-cursor-assistant`, `rf-magento-cursor-assistant`) are **submitted to the Cursor marketplace during the June cutover-prep window**. Prerequisite for that submission, and for the local-install fallback being usable on flip day: the **sister-repo `github.com/umbraculum-dev/umbraculum-toolset`** — which hosts those four plugins' source — flips visibility from private → public in the **same atomic moment as this repo's own public flip**. (The toolset repo is already hosted under the `umbraculum-dev` GitHub org, so no ownership transfer is needed; only its visibility flips.) The [`CURSOR-PLUGINS.md`](CURSOR-PLUGINS.md) §Install script clones from that URL, and the marketplace listings point at it as the canonical source — both consumers require the URL to resolve publicly from flip day onward. The operational pairing of the two visibility flips is recorded in [`ROADMAP.md`](ROADMAP.md) §"Late H1 / July 2026" Week 3 Stage 2. The public-alpha procedure is **COMPLETE only when the four marketplace listings are live** (Cursor-side approval timing — days to weeks; outside this project's control). Until the listings publish, the local-install fallback documented in [`CURSOR-PLUGINS.md`](CURSOR-PLUGINS.md) remains the canonical install path and [`AGENTS.md`](../AGENTS.md)'s apparatus self-check instructs new contributors via that fallback; once they publish, the canonical-for-now install path flips to marketplace-first across all three docs ([`CURSOR-PLUGINS.md`](CURSOR-PLUGINS.md), [`GETTING-STARTED.md`](GETTING-STARTED.md), [`AGENTS.md`](../AGENTS.md)) and the *post-marketplace-flip*-gated upgrades — the machine-readable `.cursor/required-plugins.json` manifest, the CI parity check that pins agent-side and CI-side plugin versions in lockstep, and the [`AGENTS.md`](../AGENTS.md) version-pinned (rather than presence-only) check — are unblocked. This commitment is **a single architectural commitment**, not a marketing milestone: the umbraculum-toolset apparatus is the equalizer that keeps the contribution bar low (per [`MANIFESTO.md`](../MANIFESTO.md) §1.3 and §2.2), and "the apparatus is one-click-installable from the same marketplace every Cursor user already uses" is what *operationally* discharges that commitment. The operational date for submission and the listings-live tail-tracking discipline live in [`ROADMAP.md`](ROADMAP.md) §"Late H1 / July 2026"; the present subsection is the architectural record of the closure criterion itself.
