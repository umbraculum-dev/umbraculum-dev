...
| 13 | Project callout (`> [!NOTE]` block with Umbraculum positioning) in module READMEs | `module-readme-verification` checker + `DOCS-README-STANDARDS.md` | ❌ FAIL → ✅ PASS after verifier fix-up | — | — |
...
1. **Enumerate every gate skill to run, by name, at each phase
   ...
   `packages/canonical/pim/contracts/README.md` and on `services/api/src/modules/pim/README.md`;
   run `typescript-strict-flag-verification` on `packages/canonical/pim/contracts/`;
...
9. **Specify the web-route shape (β disciplines) explicitly at plan
   time, not as a "match the canonical reference" handwave.** Run 1's
   silent `pim/` (URL-axis) choice was decision-by-default that
   happened to be correct only because the `(automation)/` reference
   was itself broken. The Week-1 audit ratified two β disciplines (no
   `(<code>)/page.tsx`, no `(<code>)/[<dynamic>]/page.tsx` at the
   route-group root) and a `registerWebModule({ ownedUrlSegments })`
   contract enforced by `scripts/check-web-url-segments.ts` in CI.
   Plans for future canonical-module web slices must (a) cite plugin
   rule `46-web-route-shape.mdc` from
   `umbraculum-platform-tsjs-cursor-assistant`, (b) enumerate the
   static sub-segments the module owns, (c) require a
   `registerWebModule()` call alongside `registerModule()`, and
   (d) require running `npm run check-web-url-segments` at the
   web-phase boundary. *(Source: Run 1 — WRONG row 7 follow-on; Week-1
   audit closing this as a structural rule rather than a per-run
   surprise.)*

These nine bullets are the **minimum extra-specification budget**
...
| B | `rules/43-non-frontier-executor-fitness-tracker.mdc` in the `umbraculum-toolset-common` Cursor plugin | Plugin source repo (`$TOOLSET_ROOT` — your clone of `github.com/umbraculum-dev/umbraculum-toolset`); plugin version bumped `0.6.0 → 0.7.0` | After `git pull` in umbraculum-toolset and **Developer: Reload Window** in affected workspaces (hook loads from source — no rsync). Cross-repo: any sibling Umbraculum repo that adopts the toolset gets the rule when the hook registers common for that workspace. |
