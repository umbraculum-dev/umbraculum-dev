...
The consultant has access to a closed set of **read-scope tools** registered by the installed canonical modules and the brewery vertical configuration. The model decides when to call which tool based on the user's question. Today's registered tools, all `scope: "read"`:
...
All tools enforce workspace membership at the service layer ([`requireActiveWorkspace`](../services/api/src/plugins/requestContext.ts)) — the AI cannot read data outside the operator's active workspace, even if it tries to construct a tool call that would cross workspaces. Tool calls and results are visible in the chat surface for transparency.

The shape of these tools is documented in the typed `AiTool` / `AiToolRegistry` contract in [`@umbraculum/contracts`](../packages/contracts/README.md); the orchestrator that runs them lives at [`services/api/src/services/ai/orchestrator.ts`](../services/api/src/services/ai/orchestrator.ts).
...
- **No autonomous writes.** The consultant cannot mutate workspace data. Write proposals follow the human-in-the-loop pattern ([`PLATFORM-ARCHITECTURE.md`](PLATFORM-ARCHITECTURE.md) §6.5): the AI drafts a proposed change, the operator confirms or rejects, only then does the platform write. Autonomous write tools are not in v0 or v1 ([`ROADMAP.md`](ROADMAP.md) "H2 2026" section).
...
- **No tools from canonical modules that have not shipped yet.** `mrp`, `wms`, `crm`, `crp`, `pim` are reserved canonical codes but have no surface (and therefore no tools) today. They are the second tranche per [`ROADMAP.md`](ROADMAP.md) "H1 2027 — second-vertical & multi-module discipline".
...
Tools are **owned by their respective modules**, not by the AI consultant itself. The intended contract is [`registerModule({ aiTools: [...] })`](../packages/module-sdk/README.md) — a canonical module or vertical configuration declares its tools alongside its routes, Prisma schema, and tier limits, and the platform's single tool registry picks them up at boot.

Today's implementation is intentionally simpler than the target. From [`toolRegistry.ts`](../services/api/src/services/ai/toolRegistry.ts):

> *"v0 instantiates one registry at API boot in `app.ts` and registers all brewery tools synchronously. Module-pluggable registration (per docs/PLATFORM-ARCHITECTURE.md §6.2) is Sprint #3+ work."*

So in v0:

- Tool implementations co-locate under [`services/api/src/services/ai/tools/<module>/`](../services/api/src/services/ai/tools/) (today: `brewery/`, `automation/`).
- The api's [`app.ts`](../services/api/src/app.ts) wires them into the in-memory registry at boot.
- The contract for tools is already public and typed (`AiTool`, `AiToolRegistry`, `AiToolScope` in [`@umbraculum/contracts`](../packages/contracts/README.md)).

The migration to module-owned registration via `registerModule({ aiTools })` is tracked under [`PLATFORM-ARCHITECTURE.md`](PLATFORM-ARCHITECTURE.md) §4.3 ("Module-pluggable target"). Third-party module authors building against the SDK should expect that contract to land before they ship; until then the contract surface (`AiTool` shape, registry interface) is stable and pinnable.
...
- **Trajectory**
  - [`ROADMAP.md`](ROADMAP.md) — "H2 2026 — AI consultant as cross-module connective tissue" + Standing principles
