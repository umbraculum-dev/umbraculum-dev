...
This is the v0 of what [`PLATFORM-ARCHITECTURE.md`](PLATFORM-ARCHITECTURE.md) §6.5 ("Safety") and §4.3 ("Sub-system") frame as the **moat** — recall that compounds over time, is workspace-scoped (not cross-tenant), and is not transferable to a competitor's AI feature even if they ship identical tools. **Product-doc RAG (Layer C D1)** is also shipped: public help and surface summaries are embedded in Postgres via **pgvector** (`ai.doc_chunks`) and retrieved through `platform.searchProductDocs` (see [`docs/design/canonical-ai-rag-surface.md`](design/canonical-ai-rag-surface.md)). Per-workspace **activity-timeline RAG (D2)** and memory/RAG unification (D3) remain v1.5 work; workspace memory remains the operator-specific half of the arc that compounds from a workspace's first paid month.
