...
| Stricter flags rollout (Phases 6a–6h) | ✅ **Complete.** All six candidate flags (`noImplicitOverride`, `noPropertyAccessFromIndexSignature`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `verbatimModuleSyntax`, `isolatedModules`) enabled across all 13 CI-gated workspaces, plus `verbatimModuleSyntax` added to the 2 non-gated workspaces (packages/platform/ui already had matching `import type` discipline from the ESLint slice; apps/web already had `isolatedModules: true` as a Next.js convention). The types-hardening slice is feature-complete. |
...
| `tsc --noEmit` enforced in CI | ✅ All 13 currently-clean workspaces are gated by `.github/workflows/typecheck.yml` (Phase 5, 2026-05-18). `apps/web` is OUT by explicit Phase 4 decision (585 errors are Tamagui-accepted cost per TAMAGUI.md). `packages/platform/ui` is OUT for the same reason (25 errors). |
...
| `packages/platform/ui` | **25** | ~4.7s | Tamagui shorthand props (`marginTop`, `gap`) + Tamagui `ThemeName "alt2"` (theme-token caveat from TAMAGUI.md §"Caveat 1"). All errors are in the Tamagui-accepted class. |
...
| `packages/platform/api-client` | 0 | ~1.5s |
| `packages/verticals/brewery/beerjson` | 0 | ~1.9s |
| `packages/platform/contracts` | 0 | ~1.8s |
| `packages/platform/i18n` | 0 | ~1.4s |
| `packages/platform/i18n-react` | 0 | ~1.5s |
| `packages/platform/media` | 0 | ~1.5s |
| `packages/platform/navigation` | 0 | ~2.4s |
| `packages/verticals/brewery/recipes-ui` | 0 | ~3.8s |
| `packages/platform/test-mcp` | 0 | ~1.8s |

`packages/verticals/brewery/core` is intentionally JavaScript-only (no `tsconfig.json`); it has its own L1 vitest coverage ([`docs/TESTING.md`](TESTING.md) Phase 5a) and is consumed as `.js`. Not in scope for `tsc --noEmit` gating.
...
| `packages/platform/api-client` | 0 ✅ | 0 ✅ | 0 ✅ | 3 ⚠️ | 0 ✅ | 0 ✅ |
| `packages/verticals/brewery/beerjson` | 0 ✅ | 132 ⚠️ | 0 ✅ | 4 ⚠️ | 0 ✅ | 0 ✅ |
| `packages/platform/contracts` | 0 ✅ | **472** ⛔ | 16 ⚠️ | 14 ⚠️ | 0 ✅ | 0 ✅ |
| `packages/platform/i18n` | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ |
| `packages/platform/i18n-react` | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ |
| `packages/platform/media` | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ |
| `packages/platform/navigation` | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ | 0 ✅ |
| `packages/verticals/brewery/recipes-ui` | 0 ✅ | 0 ✅ | 0 ✅ | 3 ⚠️ | 0 ✅ | 0 ✅ |
| `packages/platform/test-mcp` | 0 ✅ | 12 ⚠️ | 1 ⚠️ | 1 ⚠️ | 0 ✅ | 0 ✅ |
| `packages/platform/ui` | 0 ✅ | 0 ✅ | 26 (above 25 Tamagui) ⚠️ | 31 (above 25) ⚠️ | 0 (above 25) ✅ | 0 (above 25) ✅ |
...
- Does not change `packages/verticals/brewery/core` from JavaScript to TypeScript. `packages/verticals/brewery/core` is consumed as `.js` by both web and native and has L1 vitest coverage; converting it is out of scope for this slice.
...
docker compose exec -T web bash -lc \
  'cd /packages/platform/contracts && /packages/node_modules/.bin/tsc -p tsconfig.json --noEmit'
```

### A workspace that isn't mounted in the running containers

`apps/native`, `apps/web/e2e`, `services/api/prisma`, `packages/platform/api-client`, and `packages/platform/test-mcp` aren't mounted in the long-running dev containers. Use a one-off `node:20-slim` container that mounts the entire monorepo:
...
```bash
docker run --rm -v "$PWD:/repo" -w /repo node:20-slim bash -lc '
  cd /repo/packages/verticals/brewery/recipes-ui
...
- **Don't migrate `packages/verticals/brewery/core` to TypeScript "while you're in there."** It's intentionally JS, consumed by both web and native; the conversion has cross-cutting native-bundle implications out of scope for this slice.
...
- [`docs/TAMAGUI.md`](TAMAGUI.md) — the dominant type-error class in `apps/web` and `packages/platform/ui`. **TAMAGUI.md is the SoT for what to do with Tamagui-driven type errors;** this doc only references its position.
