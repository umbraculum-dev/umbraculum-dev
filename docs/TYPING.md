...
| Stricter flags rollout (Phases 6a–6g) | ✅ Complete for the 13 CI-gated workspaces. All four candidate flags (`noImplicitOverride`, `noPropertyAccessFromIndexSignature`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`) enabled across all 13 gated workspaces. The 2 non-gated workspaces (packages/ui, apps/web) stay under the Phase 4 accept-cost umbrella per [`docs/TAMAGUI.md`](TAMAGUI.md). Only Phase 6h (optional `verbatimModuleSyntax` audit) remains. |
