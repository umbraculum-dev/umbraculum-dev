...
| `tsc --noEmit` passes per-workspace | 🟡 12 of 15 workspaces green; 3 failing (624 latent type errors total — was 629 before Phase 3a). See [Per-workspace baseline](#per-workspace-baseline). |
...
### Failing baselines (3 workspaces, 624 latent type errors — was 629 before Phase 3a)
...
| `services/api` | **14** | ~7.9s | Four remaining production-code classes (after Phase 3a fixed the 5 test-authoring-related errors): (a) Prisma `Json` column typing on `metadataJson` in `brewdaySettingsService.ts` + `inventoryService.ts` (4 errors); (b) `UnitConversionWarningV1` shape mismatch in `recipesImportService.ts` (2 errors); (c) Prisma `Where` clause structural mismatch in `routes/ingredients.ts` (4 errors — `OR` array readonly-tuple branches not unifying with `FermentableWhereInput`/`HopWhereInput`); (d) Prisma client de-narrowing in `brewSessionsService.ts` transaction lambdas (4 errors on lines 924, 978, 1116, 1151 — `Omit<PrismaClient, …>` missing `$on/$connect/$disconnect/$transaction/$extends` when the transaction client is passed back into a helper that expects the full client type). Phase 3b will tackle these. |
...
| **3b — Production-code remediation in services/api** | Four remaining issue classes (14 total errors): (a) Prisma JSON column typing for `metadataJson` in `brewdaySettingsService.ts` + `inventoryService.ts` (canonical patterns: explicit `Prisma.InputJsonValue` cast vs Zod-parse-then-assign — pick one and pin); (b) `UnitConversionWarningV1` shape mismatch in `recipesImportService.ts` (add missing `message` field upstream in the warning constructor or change consumer's expected shape); (c) Prisma `Where` clause unification in `routes/ingredients.ts` (readonly-tuple `OR` branches); (d) Prisma client de-narrowing in `brewSessionsService.ts` transaction lambdas (4 sites: 924, 978, 1116, 1151 — the transaction `tx` parameter typed as `Omit<PrismaClient, …>` is passed into helpers expecting full `PrismaClient`; either widen helper signatures to accept `Prisma.TransactionClient`/`Omit<…>` directly, or stop passing tx into the helpers). | 1–2 sessions | Phase 5 (services/api CI gate), Phase 6 (strict-flag rollout for services/api) |
