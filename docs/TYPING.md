...
| Stricter flags rollout (Phases 6a–6g) | 🟡 In progress. `noImplicitOverride` and `noPropertyAccessFromIndexSignature` enabled across all 14 leaf tsconfigs (Phases 6a + 6e). `noUncheckedIndexedAccess` enabled in 13/13 CI-gated workspaces (Phases 6b + 6f); 2 non-gated workspaces (packages/ui +26 above Tamagui, apps/web +624 above Tamagui) stay under the Phase 4 accept-cost umbrella. `exactOptionalPropertyTypes` enabled in 8 workspaces (Phase 6c); 5 deferred to Phase 6g. |
