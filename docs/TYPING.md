...
| `tsc --noEmit` enforced in CI | 🟡 Only `apps/native` is gated today. Phase 5 can now gate 13 of 14 TS workspaces (everything except apps/web — see Phase 4 decision). |
...
| **5 — Per-workspace CI gate** | Add a `typecheck` workflow that runs `npm run typecheck` for all workspaces with a green baseline. Conservative initial scope: gate the 13 currently-clean workspaces = 13 of 14 TS workspaces. apps/web's inclusion is the Phase-4 decision (packages/ui's inclusion stays "Tamagui-class, not gating" unless Tamagui ships better shorthand typing). Workflow runs in parallel per-workspace via one-off `node:20-slim` containers; expected CI wall time ~10s. | 1 session | Phase 6 (without a gate, strict-flag regressions silently re-accumulate) |
