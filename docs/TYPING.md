...
| **6b — `noUncheckedIndexedAccess` partial** | Enable in the 9 truly-free workspaces (`services/api/prisma`, `apps/web/e2e`, `packages/api-client`, `packages/beerjson`, `packages/i18n`, `packages/i18n-react`, `packages/media`, `packages/navigation`, `packages/recipes-ui`). Optionally fix the 5 known real-latent-bug sites in `apps/native` (`FermDataIntegrationScreen.tsx` lines 129, 131 + `WaterMashScreen.tsx` lines 629, 673, 674) + 1 site in `packages/test-mcp` inline to bring 11 workspaces into the gate. The remaining 4 workspaces (services/api 20, packages/contracts 16, packages/ui 26-above-Tamagui, apps/web 624-above-Tamagui) defer to Phase 6f. | 1 session | Phase 6c |
| **6c — `exactOptionalPropertyTypes` partial** | Enable in the 6 truly-free workspaces (`services/api/prisma`, `apps/web/e2e`, `packages/i18n`, `packages/i18n-react`, `packages/media`, `packages/navigation`). Optionally fix the small-cost workspaces (api-client 3, beerjson 4, recipes-ui 3, test-mcp 1, apps/native 6) inline to bring 11 workspaces into the gate; dominant friction is `Partial<T>` call sites with explicit-`undefined`, fixable by `delete`+spread or destination-type tweak from `field?: T` to `field?: T | undefined`. The remaining 4 workspaces (services/api 40, contracts 14, packages/ui 31-above-Tamagui, apps/web 602-above-Tamagui) defer to Phase 6g. | 1 session | Phase 6e/f/g |
| **6e — Enable `noPropertyAccessFromIndexSignature` repo-wide** | Fix the 2566 deferred-from-6a errors. Bulk-mechanical pattern: `obj.foo` → `obj['foo']` where `obj` has an index signature. Largest concentrations: `services/api` 1388 (Fastify `RouteGenericInterface` + `process.env`), `packages/contracts` 472 (Prisma model name lookups), `apps/web` 351 (Next.js dynamic routes + `process.env`), `apps/native` 185, `packages/beerjson` 132. Likely split into per-workspace sub-phases to keep individual PRs reviewable. | 3–5 sessions | — |
