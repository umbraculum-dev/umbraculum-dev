...
**Sub-plan #9 status (2026-05-19).** Scoping pass complete: see [`docs/design/brewery-scope-migration-plan.md`](design/brewery-scope-migration-plan.md) for the L1 plan (mis-classification audit + dep-graph + verification recipe + risk register) and [`docs/design/brewery-scope-migration-per-package-handoff.md`](design/brewery-scope-migration-per-package-handoff.md) for the per-slot execution checklist (14 slots: 13 packages + 1 application-workspace bundle). Slot 1 (worked example `@brewery/test-mcp` → `@umbraculum/test-mcp`) landed alongside the scoping deliverable; slots 2–14 execute serially across subsequent sessions.
