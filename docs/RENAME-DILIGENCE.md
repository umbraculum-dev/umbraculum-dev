...
**Sub-plan #9 status — CLOSED 2026-05-19.** All 14 slots shipped: see [`docs/design/brewery-scope-migration-plan.md`](design/brewery-scope-migration-plan.md) §0 banner for the closure record + slot-by-slot recaps, and [`docs/design/brewery-scope-migration-per-package-handoff.md`](design/brewery-scope-migration-per-package-handoff.md) for the per-slot execution log. Outcome: horizontal packages now live under `@umbraculum/*`; brewery-vertical packages re-scoped under `@umbraculum/brewery-*` per the §1.3 TRAP-avoidance discipline; the four application workspaces (`api`, `web`, `native`, `web-e2e`) flipped on slot 14 (closing slot). Lessons codified into the project's plugin-pack as the `package-scope-migration-preflight` skill + the `ci-parity-local-reproduction` skill + rule 72 (local-vs-CI divergence mechanisms).
