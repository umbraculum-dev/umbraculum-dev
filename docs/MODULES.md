...
| `mrp` | Wave 4 deterministic read-only alpha proof shipped — contracts + read-only API plus web-visible and fixture-backed brewery BOM/order/requirement projections; August 2026 alpha proof still in progress; mature scope still H1 2027 | Material requirements planning, production orders, work orders | AGPLv3 | [modules/canonical/mrp.md](modules/canonical/mrp.md) |
...
| `crp` | Wave 4 deterministic read-only alpha proof shipped — contracts + read-only API plus web-visible and fixture-backed resource/work-center/capacity/schedule/conflict projections; August 2026 alpha proof still in progress; mature scope still H1 2027 (paired with `mrp`) | Capacity requirements planning, resource scheduling, work-center load | AGPLv3 | [modules/canonical/crp.md](modules/canonical/crp.md) |
...
Note on "open door": the code is reserved and the folder shape is pre-committed by [RFC-0002](rfcs/0002-canonical-module-physical-layout.md). The work to design and ship each module's surface is its own future RFC or design artifact ([RFC-0001](rfcs/0001-modules-tiers-governance-and-automation-placement.md) §12.4). For `mrp` and `crp`, Wave 1 foundation artifacts now exist (`@umbraculum/mrp-contracts`, `@umbraculum/crp-contracts`, and read-only API skeletons), Wave 2 proves read-time brewery projections through those canonical surfaces, Wave 3 makes the projections visible in read-only web pages, and Wave 4 makes that read-only proof deterministic through fixture-backed E2E coverage. This is still not the full public-alpha proof. The August 2026 alpha planning/build artifacts remain [`design/mrp-crp-august-2026-co-design-plan.md`](design/mrp-crp-august-2026-co-design-plan.md), [`design/canonical-mrp-module-surface.md`](design/canonical-mrp-module-surface.md), [`design/canonical-crp-module-surface.md`](design/canonical-crp-module-surface.md), [`design/mrp-crp-wave-1-build-log.md`](design/mrp-crp-wave-1-build-log.md), [`design/mrp-crp-wave-2-brewery-projection-build-log.md`](design/mrp-crp-wave-2-brewery-projection-build-log.md), [`design/mrp-crp-wave-3-read-only-alpha-experience-build-log.md`](design/mrp-crp-wave-3-read-only-alpha-experience-build-log.md), and [`design/mrp-crp-wave-4-alpha-proof-hardening-build-log.md`](design/mrp-crp-wave-4-alpha-proof-hardening-build-log.md).
...
|---|---|---|---|---|
| `brewery` | **Shipped — reference vertical** (β file-move landed Week 1 of late-H1-2026 per [RFC-0006](rfcs/0006-amend-rfc-0002-brewery-file-move-acceleration.md), amending [RFC-0002](rfcs/0002-canonical-module-physical-layout.md) Decision D; Prisma `brewery.*` schema split still deferred; brewery-domain docs now live under the vertical folder; MRP/CRP Wave 4 read-only proof now consumes brewery source rows through canonical read models) | `automation` (shipped), `mrp` (deterministic read-only web projection proof shipped), `wms` (planned), `crm` (planned), `crp` (deterministic read-only web projection proof shipped) | AGPLv3 (this bundle); third-party verticals are author's choice | [modules/verticals/brewery/README.md](modules/verticals/brewery/README.md) |
