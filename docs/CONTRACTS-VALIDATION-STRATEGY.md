...
|---|---|---|---|---|
| **F1** | **OpenAPI generation from Zod schemas.** Implemented via `@fastify/swagger` + `fastify-type-provider-zod` `jsonSchemaTransform`. Committed artifacts: `services/api/openapi/openapi.json` (platform catalog) + `services/api/openapi/brewery.json` (reference vertical add-on); CI: `npm run openapi:check`. Canonical doc: [`API-OPENAPI.md`](API-OPENAPI.md). | **Partial (~97% ops)** — 2026-05-31 Phase C | maintainers | Platform spec: 81 paths / 105 ops (canonical + rendering + auth/workspaces + billing/ads/ai/integrations/webhooks/platform-admin). Brewery add-on: 55 paths / 70 ops (full reference vertical except documented gaps). Residual: `POST /ai/chat` SSE. Full F1 closure when SSE is documented or explicitly exempted. |
