...
|---|---|---|---|---|
| **F1** | **OpenAPI generation from Zod schemas.** Implemented via `@fastify/swagger` + `fastify-type-provider-zod` `jsonSchemaTransform` (F1 originally named `@asteasolutions/zod-to-openapi`; outcomes unchanged). Committed artifact: `services/api/openapi/openapi.json`; CI: `npm run openapi:check`. Canonical doc: [`API-OPENAPI.md`](API-OPENAPI.md). | **Partial (alpha)** — 2026-05-28 | maintainers | Alpha partial spec (~40 paths / 58 ops): canonical modules + rendering + `/health`. Full closure when PR3 platform/brewery routes are schema-backed (~80% coverage). |
