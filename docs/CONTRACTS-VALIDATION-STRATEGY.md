...
| **F5** | **Sub-plan #9 alignment (`@brewery/*` → `@umbraculum/*` rename).** The `ValidatedSchema<T>` interface is published from `@brewery/module-sdk` today and from `@umbraculum/module-sdk` at the public flip (`docs/RENAME-DILIGENCE.md` §10). Sub-plan #9 must (a) update the published interface's npm scope, (b) update the rewritten `22-*.mdc` plugin-pack rule's code blocks, (c) update the `docs/design/pr1-contracts-migration-handoff.md` + `docs/design/pr3-routes-migration-handoff.md` examples, and (d) update the spike README's code samples. | Open | sub-plan #9 owner | Mechanical rename across documentation surfaces; not blocked by the validation slice but consumed by it. |
