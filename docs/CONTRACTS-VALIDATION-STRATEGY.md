...
|---|---|---|---|---|
| **F1** | **OpenAPI generation from Zod schemas.** Implemented via `@fastify/swagger` + `fastify-type-provider-zod` `jsonSchemaTransform`. Committed artifacts: `services/api/openapi/openapi.json` (platform catalog) + `services/api/openapi/brewery.json` (reference vertical add-on); CI: `npm run openapi:check`. Canonical doc: [`API-OPENAPI.md`](API-OPENAPI.md). | **Partial (platform + brewery add-on)** — 2026-05-31 | maintainers | Platform spec: 53 paths / 72 ops (canonical + rendering + auth/workspaces). Brewery add-on: 12 paths / 21 ops (styles, equipment, inventory, brewday, recipes, export). Full closure when remaining PR3 routes are schema-backed (~80% total coverage). |
