...
1. **Criterion (1) "new complex contract" newly satisfied.** [RFC-0002](rfcs/0002-canonical-module-physical-layout.md) (Accepted 2026-05-19) commits to 5 `packages/<code>-contracts/` packages by H1 2027; [`docs/design/canonical-automation-module-surface.md`](design/canonical-automation-module-surface.md) (Accepted 2026-05-19) lands `packages/canonical/automation/contracts/` as the template for the other four. **Net: 4 new contracts packages incoming, each with its own deeply-discriminated-union + structural-drift-detection surface (the `mailbox-data.ts` MAILBOX_SPEC validator is the first worked example, at 168 hand-rolled lines).**
2. **Criterion (2) "OpenAPI requirement" partially satisfied via SDK trajectory.** [RFC-0002](rfcs/0002-canonical-module-physical-layout.md) commits `packages/sdk/module-sdk/` as a public-facing artifact to be designed with or before the second canonical module. The "third-party AI-assisted module developers" audience makes Zod's ecosystem gravity + first-party OpenAPI support materially valuable, regardless of whether an OpenAPI spec is *required* day-one.
...
`packages/platform/contracts` exposes 57 cross-process types and 8 hand-rolled `unknown → typed` parser functions. We do **not** use Zod, Valibot, Arktype, or TypeBox. The decision after HIGH-staged Phase 1 (commit `4fa663b`) is:
...
Inside `packages/platform/contracts/src`:
...
4. **Schema-driven OpenAPI.** With Zod (or TypeBox) on Fastify routes, an OpenAPI spec falls out of the route definitions automatically. Useful for the July 2026 public-alpha milestone — third-party module developers get an auto-generated API reference.
...
1. **Pick one new contract** as the experiment. Use Zod v4 (or whichever candidate). Land it in `packages/platform/contracts/src/<area>/__zod__/`. Compare ergonomics.
...
- ❌ "Zod everywhere" expanded beyond `packages/platform/contracts` and `services/api/src/routes`. The native side and web pages consume the parsers — they should not need to know about Zod.
...
|---|---|---|---|---|
| **F1** | **OpenAPI generation from Zod schemas.** Implemented via `@fastify/swagger` + `fastify-type-provider-zod` `jsonSchemaTransform`. Committed artifacts: `services/api/openapi/openapi.json` (platform catalog) + `services/api/openapi/brewery.json` (reference vertical add-on); CI: `npm run openapi:check`. Canonical doc: [`API-OPENAPI.md`](API-OPENAPI.md). | **Done — Phase D — 2026-06-01** | maintainers | Platform spec: 81 paths / 105 ops. Brewery add-on: 55 paths / 70 ops. **175 documented ops (~97%).** Explicit exemption: `POST /ai/chat` SSE — documented in [`API-OPENAPI.md`](API-OPENAPI.md) §Streaming endpoints, not counted as OpenAPI op. Coverage floors enforced in `services/api/src/tests/openapi.test.ts`. |
...
| **F4** | **DOCS-README-STANDARDS update.** Add a required `## Validation` section to the canonical module-README template in `docs/DOCS-README-STANDARDS.md` v1.2 (next version). New modules' READMEs must state which Zod schemas they own + where the schemas live + where the paired tests live. `scripts/docs/check-readmes.py` then enforces the structural presence. | Open | docs-slice owner | Co-update with the post-PR 1 docs sweep. Mirror the section in the rewritten existing module READMEs (`packages/platform/contracts/README.md`, `packages/canonical/automation/contracts/README.md`, `packages/sdk/module-sdk/README.md`). |
| **F5** | **Sub-plan #9 alignment (`@brewery/*` → `@umbraculum/*` rename) — DONE.** The `ValidatedSchema<T>` interface is published from `@umbraculum/module-sdk` (workspace `name` field flipped 2026-05-19 in slot 11 of sub-plan #9; the public-flip target is now the live state, not a future state). Slot 11 covered (a) the published interface's npm scope, (b) any inline `22-*.mdc` plugin-pack-rule code-block references caught by the canonical bulk sweep, (c) the `pr1`/`pr3` migration-handoff examples (handoff docs are doc-tier, swept by the same sed), and (d) any spike-README code samples (any remaining occurrences live in archived spike artifacts). The two simultaneous transient cross-scope states opened by slot 9 (`@brewery/api-client` half + `@brewery/module-sdk` half) are now both CLOSED — the multi-state version of upstream-then-downstream-rename ordering held in practice across slots 9→{10,11} with no observed interaction. | **Done (2026-05-19, slot 11 of sub-plan #9)** | sub-plan #9 owner | Mechanical rename across documentation surfaces was the original scope; `@umbraculum/module-sdk` is now the live workspace name + dist + import path everywhere. See [`docs/design/brewery-scope-migration-plan.md`](design/brewery-scope-migration-plan.md) §6.11 for the slot-11 recap; remaining sub-plan #9 work is slots 12–14 (brewery-vertical packages + application-workspace bundle), which do NOT touch the validation slice. |
| **F6** | **Plugin-pack publication.** The rewritten `22-typescript-contracts-runtime-validation.mdc` rule, the new `zod-schema-scaffold` skill, and the new `contracts-zod-auditor` subagent live in `<umbraculum-toolset>/cursor-plugins/umbraculum-node-react-cursor-assistant/` (hook-loaded from source). Publication to a wider plugin-pack registry (when the project's Cursor plugin distribution strategy lands) needs (a) the rule's globs / patterns audited against external repos, (b) the skill's "absolute path" cross-references made relative, (c) the subagent's read-first list verified across the broader corpus. | Open | plugin-pack owner | Tracked alongside the §8 proposals from foundation-hardening v2.0 that already shipped (the new ones for the validation slice get the same publication discipline). |
| **F7** | **Native bundle audit (RFC-0003 F7).** Measure the `apps/native` (Expo / React Native) bundle delta with Zod adoption end-to-end (after PR 3 lands all route migrations). Paper-design estimate during the spike was ~5 KB gzipped (Zod v4 + zod/v4-mini tree-shake), well under the per-criterion threshold from the v1.x audit (≤2 KB threshold was retired during the v2.0 flip per the audit's `5.3` revision). F7 confirms or revises the paper-design number with container-side production-build measurement on a real bundler run. | **Done — 2026-06-02** | apps/native owner | Container-side esbuild measurement (`spike/validation-library/native-e6d-facades/`): marginal E6d-native typed-facade import graph adds **867 bytes gzipped** over transport-only baseline (74,527 → 75,394 bytes gzipped; **PASS** vs RFC-0003 §B 10 KB stop condition). Full Metro production bundle not re-run; esbuild proxy captures parser/facade marginal cost after E6d-native lands. |
...
| **F9** | **Shared `expectStructuralError` / `expectFirstIssuePathStartsWith` test helpers.** Both helpers are inline-defined per-test-file today (in `packages/platform/contracts/src/auth/meResponse.test.ts` for the per-schema helper, in the PR 3 handoff for the per-route helper). Extract to `packages/platform/contracts/src/__test-helpers__/zodAssertions.ts` (or `services/api/src/tests/helpers/zodAssertions.ts` for the route-level variant) so new tests get the helper via import rather than per-file duplication. | Open | tests-slice owner | Defer until ~3 test files use the helper (currently 2). The extraction is mechanical; the timing prevents premature abstraction. |
| **F10** | **Mandate framework-level response validation in both runtimes.** 2026-05-19 architectural discussion considered three paths for catching wrong-shape 200s: Path α (runbook-layer shape-check at the public-endpoint verification gate, with optional `RESPONSE_SCHEMA` / `RESPONSE_MODEL` inputs), Path β (mandate framework-level response validation at the route-declaration level; gate stays HTTP-level), and Path γ (hybrid). **Chose Path β.** Extend [`22-typescript-contracts-runtime-validation.mdc`](https://github.com/umbraculum-dev/umbraculum-toolset/tree/main/cursor-plugins/umbraculum-node-react-cursor-assistant/rules/22-typescript-contracts-runtime-validation.mdc) so Fastify routes under `services/api/src/routes/**` MUST declare `schema: { response: { 200: SomeSchema, ... } }` for any JSON-body response; add mirror rule `22-fastapi-response-model.mdc` to `umbraculum-openplc-python-cursor-assistant` mandating FastAPI `response_model=PydanticModel` on routes under `pi-sidecar/app/**`; extend the `contracts-zod-auditor` subagent contract with a 4th check (route has response schema declared for non-trivial response bodies). The public-endpoint verification gate stays HTTP-level — no new verdict variant, no new SKIP token, no new runbook input. **Manifesto-coherent**: defects are designed-out at the route-declaration level (process-level defect prevention per [`MANIFESTO.md`](../MANIFESTO.md) epigraph), not caught at QA. **Symmetric with existing posture**: today the rule mandates request-body validation via `schema: { body }`; response validation is the missing symmetric half. **Independent scope correction co-lands**: drop the stale `Magento / PHP` runbook bullet from [`45-public-endpoint-verification.mdc`](https://github.com/umbraculum-dev/umbraculum-toolset/tree/main/cursor-plugins/umbraculum-toolset-common/rules/45-public-endpoint-verification.mdc) `## Per-ecosystem runbooks` section (umbraculum-toolset-common should only reference runbooks that ship in the umbraculum-toolset marketplace per [umbrella plan §3](../../../.cursor/plans/umbrella_ecosystem_architecture_443198a8.plan.md)). Path α (runbook-layer shape-check) considered and rejected; reasoning preserved at [`~/.cursor/plans/f10_endpoint_shape_check_51877d2b.plan.md`](../../../.cursor/plans/f10_endpoint_shape_check_51877d2b.plan.md) `## Why Path beta over Path alpha` section. | **Done — Phase E+F10 — 2026-06-01** | maintainers | Rule extension + Pi-sidecar mirror + auditor check #5 live in umbraculum-toolset. F10b route migration audit (2026-06-01): **0 JSON-200 gaps** in billing, integrations, webhooks, platform-admin, ads, brewery module routes — see §F10 appendix below. Regression: `services/api/src/tests/openapiRouteCoverage.test.ts`. |
...
### F10 appendix — response-schema migration baseline (2026-06-01)

Phase E+F10 execution audited **104** `zodApp` routes across billing, integrations, webhooks, platform-admin, ads, and brewery module route files. **Gap count: 0** JSON-200 handlers missing `schema.response` (excluding documented SSE, 204, redirect, and buffer/stream exports). Platform `HealthResponseSchema` moved to `@umbraculum/contracts` so health matches the same pattern.

| Wave | Route families | Files scanned | JSON-200 gaps |
|------|----------------|---------------|---------------|
| F10b PR1 | billing, integrations, webhooks | `billing.ts`, `integrations*.ts`, `webhooks*.ts` | 0 |
| F10b PR2 | platform-admin, ads, remaining `routes/*` | `platformAds.ts`, `platformRecipes.ts`, `ads.ts`, … | 0 |
| F10b PR3 | brewery + canonical modules | `modules/brewery/routes/**`, pim/mrp/crp/automation | 0 |

Regression guard: [`services/api/src/tests/openapiRouteCoverage.test.ts`](../services/api/src/tests/openapiRouteCoverage.test.ts). OpenAPI + `@umbraculum/api-client` codegen re-run when route schemas change (`npm run verify:openapi`).
...
- [RFC-0002](rfcs/0002-canonical-module-physical-layout.md) — canonical-module physical layout (β: `packages/<code>-contracts/` × 5 + `packages/sdk/module-sdk/`). The trigger that crossed criterion (1) of this doc into "met" status.
- [`docs/design/canonical-automation-module-surface.md`](design/canonical-automation-module-surface.md) — the first canonical-module surface design, accepted 2026-05-19; Phase A landed `packages/canonical/automation/contracts/` as the worked example of the template-pattern.
...
- [`packages/platform/contracts/src/water/parseComputeAndSave.ts`](../packages/platform/contracts/src/water/parseComputeAndSave.ts) — canonical example of a hand-rolled parser after Phase 1 cleanup; migrated to Zod in PR 1 of the migration.
- [`packages/canonical/automation/contracts/src/mailbox-data.ts`](../packages/canonical/automation/contracts/src/mailbox-data.ts) — first-worked-example of a new-pattern complex contract (168 hand-rolled lines, the criterion-1 trigger); migrated to Zod in PR 2 of the migration.
