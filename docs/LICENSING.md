...
**Document role:** public-facing rationale for the license split, commercial licensing posture, and license-change process.
...
3. **Trust with operational customers.** Enterprise operational buyers — especially in manufacturing, quality, and regulated workflows — commit to 5–10 year dependencies. They will not commit to a project they cannot audit, fork if abandoned, or self-host if they need to.
...
7. **API pages on Learn are not the same as integrator-ready documentation.** Mature ERPs can ship OData, REST, and extension APIs yet leave external developers — and their own senior consultants — unable to answer *which surface, which version, which auth* without partner oral tradition. Practitioner narrative: [`design/ecosystem-case-study-business-central.md`](design/ecosystem-case-study-business-central.md); Umbraculum response: public `@umbraculum/<code>-contracts`, module route tables, `CONTRACT_VERSION` handshakes, committed OpenAPI catalogs ([`API-OPENAPI.md`](API-OPENAPI.md); F1 **Done** in [`CONTRACTS-VALIDATION-STRATEGY.md`](CONTRACTS-VALIDATION-STRATEGY.md) §Follow-ups), and OpenAPI-derived types in [`@umbraculum/api-client`](../packages/platform/api-client/README.md) (Phase E).
8. **Trials are not the same as a safe boat for repositioning.** Developers in hard times need a clear stack, free upstream tools, and visible community — spare-time learning that can lead to employment, usually at an agency, not instant agency founding. SAP offers trials and courses; TeamSystem-style ecosystems often offer partner access only; Odoo Community largely passes the Magento-shaped test. Practitioner narratives: [`design/ecosystem-case-study-sap.md`](design/ecosystem-case-study-sap.md), [`design/ecosystem-case-study-teamsystem.md`](design/ecosystem-case-study-teamsystem.md), [`design/ecosystem-case-study-odoo.md`](design/ecosystem-case-study-odoo.md). Umbraculum response: [`GETTING-STARTED.md`](GETTING-STARTED.md), **no certification program** ([`MANIFESTO.md`](../MANIFESTO.md) §2.2), expertise discoverable by public work.
...
#### 6.2.1 npm publication status (living — last reviewed 2026-05-29)

**Are we ready to publish today?** **Yes** for the MIT SDK batch — external module authors can `npm install` the seven packages below from the public registry. First versions landed **2026-05-29** (maintainer laptop publish + OIDC trusted publishing configured for future bumps via `publish-sdk-batch.yml`). See [`docs/design/npm-sdk-publish-execution-plan.md`](design/npm-sdk-publish-execution-plan.md) SP-3 sign-off.
...
| `@umbraculum/ai-tool-sdk` | ✅ | **Published** | `0.2.0` | Leaf package (zero deps). OIDC via `publish-sdk-batch.yml`. RFC-0012 path: `packages/sdk/ai-tool-sdk/`. |
| `@umbraculum/i18n-keys` | ✅ | **Published** | `0.2.0` | Leaf package (zero deps). |
| `@umbraculum/module-sdk` | ✅ | **Published** | `0.2.0` | Registry `dependencies` on `^0.2.0` leaves; monorepo dev keeps `file:../…`. |
| `@umbraculum/automation-contracts` | ✅ | **Published** | `0.2.0` | Path: `packages/canonical/automation/contracts/`. |
| `@umbraculum/pim-contracts` | ✅ | **Published** | `0.2.0` | Path: `packages/canonical/pim/contracts/`. |
| `@umbraculum/mrp-contracts` | ✅ | **Published** | `0.2.0` | Path: `packages/canonical/mrp/contracts/`. |
| `@umbraculum/crp-contracts` | ✅ | **Published** | `0.2.0` | Path: `packages/canonical/crp/contracts/`. |
...
**Monorepo contributors** use npm workspaces (workspace symlinks to `packages/{platform,sdk,canonical,verticals}/` for in-tree SDKs; consumer manifests pin published `@umbraculum/*` at registry semver — contracts/api-client `@0.0.1`, MIT SDK batch `@0.2.0` — see [`docs/design/npm-sdk-monorepo-dogfood.md`](design/npm-sdk-monorepo-dogfood.md)). **External module repos** install from npm first; git/workspace is a fallback for unreleased pins.
