...
| `mrp` | **Wave 3 read-only alpha experience shipped** (August 2026 alpha co-design target still in progress; mature scope H1 2027) | Brew sessions now project as production orders, recipes project as BOMs, session steps project as operations, and recipe ingredients project as assumption-only material requirements through the canonical MRP read routes and read-only web pages. This is read-time projection only: brewery remains the source of truth, and no MRP write workflow is implied. |
| `wms` | Open door (H2 2027) | Brewery ingredients become catalog SKUs; the on-hand inventory becomes WMS stock-on-hand. |
| `crp` | **Wave 3 read-only alpha experience shipped** (August 2026 alpha co-design target still in progress; mature scope H1 2027) | Automation vessels now project as CRP resources, equipment profiles project as work centers, and timed brew-session steps project as scheduled operations/load/conflicts through the canonical CRP read routes and read-only web pages. This is read-time projection only: brewery/automation remain the source of truth, and no CRP write workflow or optimizer is implied. |
