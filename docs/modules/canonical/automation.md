...
| **Web** | [`apps/web/app/[locale]/(automation)/`](../../../apps/web/app/[locale]/(automation)/) | **Shipped (B-3)** — `page.tsx` (vessel list), `[vesselCode]/page.tsx` (vessel detail). Next.js route group; no URL prefix change. |
...
|---|---|---|---|---|
| `GET` | `/automation/vessels` | session + active workspace | `VesselListResponseSchema` (from `@umbraculum/automation-contracts`) | **Shipped** — deterministic `code asc` ordering |
| `GET` | `/automation/vessels/:code` | session + active workspace | `VesselStateResponseSchema` | **Shipped** — 404 on workspace-mismatch (no cross-workspace leakage) |
