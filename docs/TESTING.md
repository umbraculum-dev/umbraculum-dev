...
**Status:** v1.12 (test-coverage hardening pass — Phase 1 + Phase 2 + Phase 3 + Phase 4a + **all 5 Phase 4b sub-phases** + **Phase 5a (`packages/core` L1 audit + gap-fix)** + **Phase 5b-1 + 5b-2 + 5b-3 (waterCalc L1: `saltAdditions` + `residualAlkalinity` + `mashPhEstimateV1` + `mashAcidificationManual` + `mashAcidificationTargetMashPh`)** landed 2026-05-17/18; see [Coverage audit + hardening pass](#coverage-audit--hardening-pass-2026-05-17) below). The foundation-hardening pass ahead of the H1 2027 public-AGPLv3 flip ([`docs/ROADMAP.md`](ROADMAP.md)) has four slices: lint ✅ landed, **tests 🟢 in progress on the final Phase 5b sub-phases (5b-1 + 5b-2 + 5b-3 landed; 5b-4 + 5b-5 remain — see [Phase 5b implementation backlog](#phase-5b-implementation-backlog-watercalc-l1-unit-tests) below)**, types 🟡 not started, docs 🟡 not started. The "what to test" framework below is unchanged; the audit + phase log are new.
...
| **Phase 5b — `services/api/src/domain/waterCalc/*` L1 unit tests** | Split into 5 bounded sub-phases (5b-1 → 5b-5). Audit identified ~1000 LoC of pure-math/derivation code across 4 substantial files (`mashAcidificationTargetMashPh.ts` 307 LoC, `mashPhEstimateV1.ts` 215 LoC, `saltAdditions.ts` 183 LoC, `spargeAcidification.ts` 313 LoC) + 5 smaller files with no direct L1 unit tests. Currently only INDIRECTLY covered via L4 contract snapshots. **Phase 5b-1 (2026-05-18):** `residualAlkalinity.test.ts` (17 tests) + `saltAdditions.test.ts` (24 tests). **Phase 5b-2 (2026-05-18):** `mashPhEstimateV1.test.ts` (32 tests). **Phase 5b-3 (2026-05-18):** new `mashAcidificationManual.test.ts` (12 tests) pinning bracket clamps + bisection monotonicity + inverse-symmetry round-trip with `spargeAcidification` + solid-strength branch + validation throws; new `mashAcidificationTargetMashPh.test.ts` (19 tests) pinning happy-path bisection convergence + liquid/solid result-shape contract + early-return-when-below-target + monotonic-target/alkalinity sensitivities (using probe-verified converging inputs in the model's reachable range) + maltClass→modelKey wiring sensitivity (roast + acidulated reduce `estimateAtZero`) + debug payload shape + all 9 validation throws. services/api: 233 → 337 tests across 40 → 45 files. | ~1-2 days total | 🟢 **In progress** — 5b-1 + 5b-2 + 5b-3 ✅ landed 2026-05-18; 5b-4 + 5b-5 remain (see [Phase 5b implementation backlog](#phase-5b-implementation-backlog-watercalc-l1-unit-tests) below). | See [Phase 5b implementation backlog](#phase-5b-implementation-backlog-watercalc-l1-unit-tests) |

**Recommended order:** Phase 2 (L4 snapshots) ✅ done. Phase 3a (Phase 4b L5 pin) ✅ done. Phase 3b (SelectWorkspace L5 web pin for Phase 5g) ✅ done. Phase 4a (L2 route surface audit) ✅ done. Phase 4b-1 through 4b-5 ✅ done — **all 5 Phase 4b sub-phases shipped 2026-05-18**. Phase 5a (`packages/core` audit + gap-fix) ✅ done. **Phase 5b-1 + 5b-2 + 5b-3 ✅ done 2026-05-18** — services/api 233 → 337 tests across 40 → 45 files. **Remaining work:** Phase 5b-4 + 5b-5 (2 sub-phases across the remaining 4 waterCalc files; ~3-4h total). Phase 4d (deferred) waits for the ACL-wiring architectural decision.
