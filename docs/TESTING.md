...
| **L4 BeerJSON contract snapshots** | `services/api/src/tests/contracts/*.test.ts` | 5 of ~12 native-consumed endpoints covered (was 2 pre-audit): `recipe.contract.test.ts` (POST /recipes + GET /recipes/:id), `auth.contract.test.ts` (GET /auth/me), `waterProfiles.contract.test.ts` **(added 2026-05-18 as Phase 2a regression-pin for the Phase 4b bug)**, `recipeWater.contract.test.ts` (GET /water-settings + GET /water-hub-summary, added 2026-05-18). | 🟡 **Improving — Phase 2a landed 2026-05-18** (5/12); Phase 2b scoped for the remaining ~7 endpoints (3 × `compute-and-save`, brew-sessions, equipment-profiles, ingredients × 3). | Phase 4b NOW pinned at L4. |
...
| **Phase 2 — L4 contract snapshots for the uncovered native-consumed endpoints** | Audit `apps/native/src/**` for `api.{get,post,patch,put,delete}` call sites; cross-reference against existing `*.contract.test.ts` files; author the missing snapshot tests. Split into two sub-phases for bounded commits. | ~2-4 hours total (~15-30 min each) | 🟢 **Phase 2a landed 2026-05-18** (3 of 10 snapshots: `/water-profiles`, `/recipes/:id/water-settings`, `/recipes/:id/water-hub-summary`). Phase 2b scoped (see [Phase 2b backlog](#phase-2b-backlog-l4-snapshots-still-missing) below). | This doc § Phase 2b backlog |
...
#### Phase 2b backlog (L4 snapshots still missing)

Endpoints that `apps/native/` consumes but that do not yet have a `.contract.test.ts`. Authored as a checklist so a future Phase 2b commit can chew through them in one focused pass.

| Endpoint | Native call site (example) | Companion L1 parser test (if any) | Notes |
|---|---|---|---|
| POST `/recipes/:id/water-settings/mash/compute-and-save` | `apps/native/src/screens/WaterMashScreen.tsx` | ✅ `packages/contracts/src/water/parseComputeAndSave.test.ts` (mash variant) | Fixture: recipe + workspace water profile + `mashAcidificationMode: "targetPh"` payload. |
| POST `/recipes/:id/water-settings/sparge/compute-and-save` | `apps/native/src/screens/WaterSpargeScreen.tsx` | ✅ same file (sparge variant) | Same fixture pattern as mash. |
| POST `/recipes/:id/water-settings/boil/compute-and-save` | `apps/native/src/screens/WaterBoilScreen.tsx` | ✅ same file (boil variant) | Same fixture pattern. |
| GET `/recipes/:id/brew-sessions` | `apps/native/src/screens/BrewSessionsListScreen.tsx` | — (no L1 parser yet) | Likely empty-or-non-empty list; fixture needs at least one brew session created. |
| POST `/recipes/:id/brew-sessions` | `apps/native/src/screens/BrewSessionsListScreen.tsx` | — | Snapshot the creation response shape. |
| GET `/brew-sessions/:id` | `apps/native/src/screens/BrewSessionDetailScreen.tsx` | — | Detail response shape (likely includes integrations metadata; biggest of the brew-session three). |
| GET `/equipment-profiles` | `apps/native/src/screens/EquipmentScreen.tsx` | — | Likely simple list; quick win. |
| GET `/ingredients/fermentables` | `apps/native/src/screens/RecipeEditScreen.tsx` | — | Paged/searchable list; snapshot with no query. |
| GET `/ingredients/hops` | `apps/native/src/screens/RecipeEditScreen.tsx` | — | Same as fermentables. |
| GET `/ingredients/yeasts` | `apps/native/src/screens/YeastScreen.tsx` + `RecipeEditScreen.tsx` | — | Same as fermentables. |

Estimate: ~2-3 hours for the full Phase 2b pass. Recommend splitting into "water (3 compute-and-save)" + "brew-sessions (3)" + "list endpoints (4)" sub-commits to keep diffs reviewable. The 3 `compute-and-save` snapshots are the highest-value next chunk because they have companion L1 parser tests already in place, so the L1+L4 alignment becomes complete for the entire water-calc surface.
