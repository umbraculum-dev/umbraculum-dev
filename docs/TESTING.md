...
| **L4 BeerJSON contract snapshots** | `services/api/src/tests/contracts/*.test.ts` | 8 of ~12 native-consumed endpoints covered (was 2 pre-audit): `recipe.contract.test.ts` (POST /recipes + GET /recipes/:id), `auth.contract.test.ts` (GET /auth/me), `waterProfiles.contract.test.ts` **(added 2026-05-18 as Phase 2a regression-pin for the Phase 4b bug)**, `recipeWater.contract.test.ts` (GET /water-settings + GET /water-hub-summary, added 2026-05-18), `recipeWaterCompute.contract.test.ts` (POST mash + sparge + boil compute-and-save, added 2026-05-18 as Phase 2b first chunk — closes L1+L4 alignment for the entire water-calc surface). | 🟡 **Improving — Phase 2a + 2b/water landed 2026-05-18** (8/12); Phase 2b remaining: 7 endpoints (brew-sessions × 3, equipment-profiles, ingredients × 3). | Phase 4b pinned at L4; entire water-calc surface has L1+L4 alignment. |
...
| **Phase 2 — L4 contract snapshots for the uncovered native-consumed endpoints** | Audit `apps/native/src/**` for `api.{get,post,patch,put,delete}` call sites; cross-reference against existing `*.contract.test.ts` files; author the missing snapshot tests. Split into sub-phases for bounded commits. | ~2-4 hours total (~15-30 min each) | 🟢 **Phase 2a + Phase 2b/water landed 2026-05-18** (6 of 10 snapshots — full water-calc surface now has L1+L4 alignment). Phase 2b remaining: 7 endpoints (brew-sessions × 3, equipment-profiles × 1, ingredients × 3). See [Phase 2b backlog](#phase-2b-backlog-l4-snapshots-still-missing) below. | This doc § Phase 2b backlog |
...
Remaining estimate after the water-calc chunk landed: ~1.5-2 hours for the 7 remaining endpoints. Recommended split: "brew-sessions (3)" + "list endpoints (4)" sub-commits to keep diffs reviewable.

Closed sub-phase **2b/water (2026-05-18)** — the 3 `compute-and-save` snapshots landed together because they share a single fixture (recipe + workspace water profile) and have companion L1 parser tests at `packages/contracts/src/water/parseComputeAndSave.test.ts`. The entire water-calc surface (mash/sparge/boil compute-and-save + water-hub-summary + water-settings + water-profiles) now has explicit L1+L4 alignment, so any future contract change to the water wire format will surface in PR diffs from both sides.
