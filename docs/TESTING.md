...
### L1 - Unit (`packages/platform/contracts`, `packages/verticals/brewery/core`)

Pure functions. No DB, no Fastify, no network. If you add or rename a field in:

- [packages/platform/contracts/src/water/parseComputeAndSave.ts](../packages/platform/contracts/src/water/parseComputeAndSave.ts)
- [packages/platform/contracts/src/water/parseHubSummary.ts](../packages/platform/contracts/src/water/parseHubSummary.ts)
- [packages/platform/contracts/src/analysis/parseGravityAnalysis.ts](../packages/platform/contracts/src/analysis/parseGravityAnalysis.ts)
- [packages/verticals/brewery/core/src/gravity.js](../packages/verticals/brewery/core/src/gravity.js) or [packages/verticals/brewery/core/src/units](../packages/verticals/brewery/core/src/units)
...
`platform/dashboard.spec.ts` skips the axe rule `aria-valid-attr-value` because `@tamagui/accordion@2.2.0` still ships an id-wiring bug (re-tested 2026-06-10 during the 2.2.0 monorepo bump): every `Accordion.Trigger` renders `aria-controls="<contentId>"` but the corresponding `Accordion.Content` DOM node never receives that id (the trigger and content sides each call `React.useId()` independently — see `node_modules/@tamagui/accordion/src/Accordion.tsx:407`). The interaction works for keyboard and screen readers (`aria-expanded`, `data-state` are correctly wired), but axe still flags the trigger as critical.
...
2. Remove the `skipFailures: ["aria-valid-attr-value"]` argument from `apps/web/e2e/platform/dashboard.spec.ts`.
...
`@tamagui/radio-group@2.2.0`'s "native mode" wraps a real `<input type="radio">` for accessibility, but the change event from that native input does **not** propagate back to the group's `onValueChange` callback. The visible bullet moves on click (because the native input toggles its own `checked` attribute), but React state never flips — so any conditional UI keyed on the selected value (button labels, dependent inputs) stays frozen.

We hit this in `packages/platform/ui/src/primitives/ModeFieldset.tsx`, which used to set `native={isWeb}`. Symptom on the mash/sparge/boil water pages: clicking "Manual acid amount" left the submit button reading "Calculate & save snapshot" and never revealed the "Acid added" input. Fixed by dropping `native={true}` (kept the Tamagui-native render path; ARIA + label wiring is unchanged). Covered by the regression test `apps/web/e2e/verticals/brewery/water-calc.spec.ts` → "mash acidification mode radio actually flips React state on web".
...
- **MCP server** that smooths the non-UI prerequisites (smoke / seed / `loginAs`): [packages/platform/test-mcp](../packages/platform/test-mcp) + the `test-mcp-server` skill shipped by the `umbraculum-node-react-cursor-assistant` plugin (`skills/test-mcp-server/SKILL.md` in the plugin install).
...
| **L5 Web E2E** | `apps/web/e2e/**/*.spec.ts` | **11 specs** in `platform/` (4), `canonical/` (2), `verticals/brewery/` (5) | 🟢 **RFC-0011 Wave 5 (2026-06-07)** — folder taxonomy mirrors product layers. Phase 4b production-rendering symptom pinned at L5 (`verticals/brewery/water-profiles.spec.ts`, 2 tests). Phase 5g SelectWorkspace flow pinned at L5 (`platform/select-workspace.spec.ts`, 3 tests). | Phase 4b ✅ pinned L1+L4+L5; Phase 5g ✅ pinned L5 (apps/web side; native side deferred until apps/native testing infra). |
...
|---|---|---|---|
| ~~POST `/recipes/:id/water-settings/mash/compute-and-save`~~ | `apps/native/src/screens/WaterMashScreen.tsx` | ✅ `packages/platform/contracts/src/water/parseComputeAndSave.test.ts` (mash variant) | ✅ **Landed 2026-05-18** (`recipeWaterCompute.contract.test.ts`). |
...
1. Inventoried every `.js` source file in `packages/verticals/brewery/core/src/**` (4 files, ~50 LoC of code).
2. Inventoried every export (functions + constants) from those files via the public surface declared in `packages/verticals/brewery/core/src/index.d.ts`.
...
1. **Highest-signal gap (closed by Phase 5a): `DEFAULT_MASH_TARGET_PH` was un-pinned.** The constant is documented in `src/water.js` as coupled to the Prisma column default on `recipe_water_settings.mash_target_ph`. Without a test, a future PR that changes the constant in `packages/verticals/brewery/core` without updating the Prisma migration creates **silent drift** between the core default and the database default. Pinned by new `src/water.test.js` (2 tests).
...
|---|---|:-:|
| `packages/verticals/brewery/core/src/water.test.js` | **New** — pins `DEFAULT_MASH_TARGET_PH` value + sensible-window guard (5.0–5.8). Comment in test points reviewers at the Prisma migration as the linked change. | +2 |
| `packages/verticals/brewery/core/src/gravity.test.js` | Extended with `describe("platoToSg upper boundary (Phase 5a)")` (3 tests) + `describe("sgToPlato upper-bound behavior (Phase 5a)")` (2 tests). | +5 |
| `packages/verticals/brewery/core/src/units/units.test.js` | Extended with `describe("Phase 5a contract pins")` (4 nested describes, 11 tests total covering negative passthrough, non-defensive helpers, strict-string matching, non-string unit guards). | +11 |
| `packages/verticals/brewery/core/src/units/rounding.test.js` | Extended with 3 new pins (negative half-rounding direction, negative higher-decimal precision, zero-input behavior). | +3 |
| **Total** | | **+21 tests** |
