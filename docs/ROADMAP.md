...
- **Foundation hardening pass (lint, types, tests) lands ahead of the public AGPLv3 flip.** HIGH-staged ESLint completed in May 2026 (ahead of the H2 2026 target) — both `no-explicit-any` and `no-unused-vars` are now `error` repo-wide and the monorepo is warning-free under the current rule set. HIGH-full (type-aware rules + `--max-warnings 0` everywhere) is now scoped as a 5-phase plan in [`docs/LINTING.md`](LINTING.md), informed by a measurement run that surfaced 1,671 type-aware warnings (~25% auto-fixable, full-repo lint at 44s wall) and showed the friction is concentrated in `services/api` boundaries and the `apps/web` Tamagui surface, not in `apps/native`. Target: H1 2027. See [`docs/TAMAGUI.md`](TAMAGUI.md) for the Tamagui-specific adaptation strategy. The intent is that a contributor reading the codebase for the first time sees a clean lint surface, descriptive docs about known type-system caveats, and CI gates that prevent regression.
