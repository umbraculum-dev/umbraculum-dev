...
**Post-Week-3 tail — public-alpha closure criterion.** The public-alpha procedure is **COMPLETE only when all three umbraculum-dev apparatus plugins are live on the Cursor marketplace** (recorded as the architectural closure criterion in [`docs/PLATFORM-ARCHITECTURE.md`](PLATFORM-ARCHITECTURE.md) §10.1.1). Marketplace submission happens during the Week 3 cutover-prep window; marketplace approval timing is Cursor-side and may extend the procedure's tail by days to weeks. This approval tail is why the external release window is **July 2026**, not a single June date. Until the listings publish, [`AGENTS.md`](../AGENTS.md)'s apparatus self-check continues to instruct new contributors via the **workspace-scoped hook + source clone** install path per [`docs/CURSOR-PLUGINS.md`](CURSOR-PLUGINS.md) and toolset [`WORKSPACE-PLUGIN-LOADING.md`](https://github.com/umbraculum-dev/umbraculum-toolset/blob/master/cursor-plugins/docs/WORKSPACE-PLUGIN-LOADING.md). When the three listings go live, this tranche is closed end-to-end: the corresponding entry gains its `✅ Done <date>` marker, the install-path canonical-for-now flips from hook + source to **marketplace per-workspace enablement** across [`docs/CURSOR-PLUGINS.md`](CURSOR-PLUGINS.md), [`docs/GETTING-STARTED.md`](GETTING-STARTED.md), and [`AGENTS.md`](../AGENTS.md) (hook retained for plugin HEAD development only — see toolset `WORKSPACE-PLUGIN-LOADING.md` §4), and the *post-marketplace-flip*-gated work below ([`docs/CURSOR-PLUGINS.md`](CURSOR-PLUGINS.md) §"Future: minimum-version enforcement" — the `.cursor/required-plugins.json` manifest + the CI parity check + the AGENTS.md version-pinned upgrade) is unblocked.

**Public-alpha support and orientation pages.** ✅ **Phase 2 `2d` done (2026-06-26).** [https://umbraculum.dev/support/](https://umbraculum.dev/support/) is live with **Liberapay** (recurring) and **Buy Me a Coffee** (one-time); in-kind via forum per [`donation-channels.md`](design/donation-channels.md). Brochure and `/support` are **search-indexable** since **2c (2026-06-27)**. Principles: [`CORE-DEVELOPMENT-AND-COMMUNITY.md`](CORE-DEVELOPMENT-AND-COMMUNITY.md) §5. Further channels (GitHub Sponsors, Open Collective) require a community vote when runbook triggers fire.

**Public-alpha production hosts.** ✅ All production URLs live. **[forum.umbraculum.dev](https://forum.umbraculum.dev/)** (2026-06-08), **[demo.umbraculum.dev](https://demo.umbraculum.dev/)** (2026-06-03), **[umbraculum.dev](https://umbraculum.dev/)** + **[docs.umbraculum.dev](https://docs.umbraculum.dev/)** indexed since **2c (2026-06-27)**. See [`design/production-hosts.md`](design/production-hosts.md), [`community-forum-runbook.md`](design/community-forum-runbook.md), [`demo-host-runbook.md`](design/demo-host-runbook.md). **Remaining post-flip tail:** Cursor marketplace listings live (**C6**); see flip runbook §11.
...
| `@umbraculum/contracts` | ✅ | ✅ `0.0.1` (2026-06-02) |
| `@umbraculum/api-client` | ✅ | ✅ `0.0.1` (2026-06-02) |
...
| **F-mod** | **Optional reference vertical / core installation profile** — brewery opt-in like Magento Sample Data | `WorkspaceBillingAddon` + module install model ([RFC-0009](rfcs/0009-workspace-billing-addons-and-entitlements.md)); boot composition ([RFC-0002](rfcs/0002-canonical-module-physical-layout.md) §3 plugin-driven registration) | H1 2027 mature scope | Self-host **core profile** without brewery routes/nav; workspace omit via add-on rows; AI + shell respect installed module set | **Waves A–F shipped 2026-06** (manifest, build/CI, native blank app, Prisma strip, entitlements prep) — [`installation-profile.md`](design/installation-profile.md), [`BUILDING-YOUR-VERTICAL.md`](BUILDING-YOUR-VERTICAL.md) |
...
| **1g** | **Docs-site pre-flip SEO** (`noIndex`, `robots.txt`) | **Removed at 2c ✅ 2026-06-27** | Was shipped pre-flip; gates removed at flip | `docs-site/docusaurus.config.ts`, `static/robots.txt` |
| **1h** | **Support page scaffold** (brochure `/support/`) | **Shipped** | Live at [umbraculum.dev/support/](https://umbraculum.dev/support/) — **2d ✅ 2026-06-26** | [`umbraculum-brochure` `public/support/`](https://github.com/umbraculum-dev/umbraculum-brochure), [`donation-channels.md`](design/donation-channels.md) |
...
**Phase 1 complete** except **1f** (deferred H1 2027). **Phase 2 `2c` ✅ 2026-06-27** — post-flip tail **C6–C9** in [`public-alpha-flip-day-runbook.md`](design/public-alpha-flip-day-runbook.md) §11.

**Flip gates (2026-06-27):** **2c ✅** — three source repos public; `noindex` removed; GitHub Release **`v0.0.1-alpha`**; DocSearch live (**C5**). **2d ✅** — Liberapay + Buy Me a Coffee at [umbraculum.dev/support/](https://umbraculum.dev/support/). **Forum ✅** — [forum.umbraculum.dev](https://forum.umbraculum.dev/). **Demo ✅** — [demo.umbraculum.dev](https://demo.umbraculum.dev/). **Tail:** marketplace listings live (**C6**); post-flip smoke (**C7–C9**) — flip runbook §11.
...
| **2c** | **Stage 2 atomic flip** — **three source repos** public (`umbraculum-dev`, `umbraculum-toolset`, `umbraculum-brochure`), `v0.0.1-alpha`, toolset marketplace submit, **Cloudflare deploy** (remove `noindex`) | **✅ 2026-06-27** — repos public; tag + **GitHub Release**; SEO gates; C2/C5 in flight; C3/C6–C9 remain | [`public-alpha-flip-day-runbook.md`](design/public-alpha-flip-day-runbook.md), [`public-alpha-cloudflare-pages-runbook.md`](design/public-alpha-cloudflare-pages-runbook.md) |
...
| **2e** | **Launch comms** — publish announcement; cross-post forum; optional sponsors list seed | **Partial ✅ 2026-06-27–28** — GitHub Release + forum [topic/84](https://forum.umbraculum.dev/t/umbraculum-public-alpha-open-source-toolset-for-workspace-shaped-operational-applications/84); optional social/brochure home link | [`PUBLIC-ALPHA-ANNOUNCEMENT.md`](PUBLIC-ALPHA-ANNOUNCEMENT.md), flip-day runbook §8 |
...
| **2g** | **DocSearch application submit** (Algolia) | **✅ Live 2026-06-27** — production search on docs | RFC-0005 P5 · **C5** |
