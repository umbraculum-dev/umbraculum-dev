...
| **Test-coverage hardening** — drive per-layer coverage gaps from the cheapest-test-layer mapping in [`docs/TESTING.md`](TESTING.md). HIGH-full surfaced two latent bugs that escaped existing tests (Phase 4b stale rename, Phase 5g render-prop typing); both indicate gaps in the L4 (contract-snapshot) and L5 (Playwright) layers respectively. | 🟢 **In progress — Phase 1 + Phase 2a + Phase 2b/water landed.** Phase 1 (2026-05-17): L1 contract parser coverage 5/8 → 8/8 (32 new tests, 70 total). Phase 2a + Phase 2b/water (2026-05-18): L4 BeerJSON contract snapshots for the entire water-calc surface — `/water-profiles` (the Phase 4b regression-pin), `/water-settings`, `/water-hub-summary`, and `/water-settings/{mash,sparge,boil}/compute-and-save` — L4 coverage 2/12 → 8/12. Phase 2b remaining (~7 endpoints — brew-sessions × 3, equipment-profiles, ingredients × 3) scoped in `docs/TESTING.md`. | [`docs/TESTING.md`](TESTING.md) → "Coverage audit + hardening pass" |
