...
| **Test-coverage hardening** — drive per-layer coverage gaps from the cheapest-test-layer mapping in [`docs/TESTING.md`](TESTING.md). HIGH-full surfaced two latent bugs that escaped existing tests (Phase 4b stale rename, Phase 5g render-prop typing); both indicate gaps in the L4 (contract-snapshot) and L5 (Playwright) layers respectively. | 🟢 **In progress — Phase 1 + Phase 2 + Phase 3 complete.** Phase 1 (2026-05-17): L1 contract parser coverage 5/8 → 8/8 (32 new tests, 70 total). Phase 2 (2026-05-18, all 3 sub-phases landed in one day): L4 BeerJSON contract snapshots for **all 15 native-consumed endpoints** (7 contract suites, 16 tests, L4 coverage 2/15 → 15/15). Phase 3a (2026-05-18): L5 Playwright regression-pin for the Phase 4b production-rendering symptom (`apps/web/e2e/smoke/water-profiles.spec.ts`; 2 tests). Phase 3b (2026-05-18): L5 Playwright regression-pin for the Phase 5g SelectWorkspace flow (`apps/web/e2e/smoke/select-workspace.spec.ts`; 3 tests; required adding `e2e-multi-admin` persona + secondary workspace seed). **Phase 4b has full L1+L4+L5 trio; Phase 5g has L5 web pin (native side deferred until apps/native gets a test runner).** Smoke suite: 13 → 18 specs, all green. Phases 4-5 (L2 audit + optional `packages/core` audit) scoped in `docs/TESTING.md`. | [`docs/TESTING.md`](TESTING.md) → "Coverage audit + hardening pass" |
...
1. **Test-coverage hardening (L4 + L5 gaps).** HIGH-full demonstrated that the lint promotion caught 2 latent bugs that should have been caught by tests. Prefer adding the missing tests over relying on lint to surface future similar drift. Phase 1 (L1 contract parser coverage) landed 2026-05-17; Phase 2 (L4 BeerJSON contract snapshots for all 15 native-consumed endpoints), Phase 3a (L5 Phase 4b pin), and Phase 3b (L5 Phase 5g SelectWorkspace pin) all landed 2026-05-18. **Both HIGH-full-surfaced latent bugs are now regression-pinned.** Remaining: Phase 4 (L2 integration coverage gap audit), Phase 5 (optional `packages/core` audit) tracked in [`docs/TESTING.md`](TESTING.md). Probably 3-5 days of focused work for the remaining phases.
