...
1. **Test-coverage hardening (L4 + L5 gaps).** HIGH-full demonstrated that the lint promotion caught 2 latent bugs that should have been caught by tests. Prefer adding the missing tests over relying on lint to surface future similar drift. Phase 1 (L1 parser coverage), Phase 2 (L4 snapshots), Phase 3a + 3b (L5 regression pins), Phase 4a (L2 route surface audit), Phase 4b-1 (brew-sessions cross-workspace isolation), and Phase 4b-2 (inventory L2 from scratch) all landed 2026-05-17/18. **Both HIGH-full-surfaced latent bugs are regression-pinned, AND the audit's two highest-priority L2 gaps (brew-sessions cross-workspace isolation + the largest zero-coverage workspace-scoped surface) are now pinned.** Remaining: Phase 4b-3 (brewday-settings L2 — small, ~30-45min), Phase 4b-4 (water compute-and-save + hub-summary auth pins — ~30min), Phase 4b-5 (platform-admin gate — ~1-1.5h), Phase 5 (optional `packages/core` audit). Phase 4d (role-based ACL coverage) is deferred until `AclService.requireRole` gets wired into routes. Tracked in [`docs/TESTING.md`](TESTING.md). Probably <1 day for Phase 4b-3 through 4b-5; Phase 5 a quick afternoon.
