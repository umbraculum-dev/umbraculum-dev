...
| **Test-coverage hardening** — drive per-layer coverage gaps from the cheapest-test-layer mapping in [`docs/TESTING.md`](TESTING.md). HIGH-full surfaced two latent bugs that escaped existing tests (Phase 4b stale rename, Phase 5g render-prop typing); both indicate gaps in the L4 (contract-snapshot) and L5 (Playwright) layers respectively. | 🟢 **In progress — Phase 1 landed 2026-05-17.** Audit + phase plan landed; L1 contract parser coverage now 8/8 (32 new tests, 70 total). Phases 2-5 scoped (L4 snapshots for 10 native-consumed endpoints; L5 regression pins for Phase 4b/5g; L2 coverage audit; optional `packages/core` audit). | [`docs/TESTING.md`](TESTING.md) → "Coverage audit + hardening pass" |
