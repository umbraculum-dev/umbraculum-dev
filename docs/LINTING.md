...
1. **Test-coverage hardening (L4 + L5 gaps).** HIGH-full demonstrated that the lint promotion caught 2 latent bugs that should have been caught by tests. Prefer adding the missing tests over relying on lint to surface future similar drift. Phase 1 (L1 contract parser coverage) landed 2026-05-17; Phase 2 (L4 BeerJSON contract snapshots for all 15 native-consumed endpoints) and Phase 3a (L5 Playwright regression-pin for Phase 4b production-rendering symptom) both landed 2026-05-18. **Phase 4b now has a full L1+L4+L5 regression-pin trio.** Remaining: Phase 3b (L5 SelectWorkspace flow pin for Phase 5g), Phase 4 (L2 integration coverage gap audit), Phase 5 (optional `packages/core` audit) tracked in [`docs/TESTING.md`](TESTING.md). Probably 1 week of focused work for the remaining phases.
