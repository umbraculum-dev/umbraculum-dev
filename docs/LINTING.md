...
| **Test-coverage hardening** — drive per-layer coverage gaps from the cheapest-test-layer mapping in [`docs/TESTING.md`](TESTING.md). HIGH-full surfaced two latent bugs that escaped existing tests (Phase 4b stale rename, Phase 5g render-prop typing); both indicate gaps in the L4 (contract-snapshot) and L5 (Playwright) layers respectively. | 🟢 **In progress — Phase 1 + Phase 2a landed.** Phase 1 (2026-05-17): L1 contract parser coverage 5/8 → 8/8 (32 new tests, 70 total). Phase 2a (2026-05-18): L4 BeerJSON contract snapshots for `/water-profiles` (the Phase 4b regression-pin), `/water-settings`, and `/water-hub-summary` — L4 coverage 2/12 → 5/12. Phase 2b checklist (~7 remaining endpoints) scoped in `docs/TESTING.md`. | [`docs/TESTING.md`](TESTING.md) → "Coverage audit + hardening pass" |
