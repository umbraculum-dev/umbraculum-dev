...
- **E — Cursor rule (`23a-eslint-fixall-discipline.mdc`):** ✅ committed upstream 5b. The rule is plugin-owned (per `11-cursor-package-files-edit-in-source-repo.mdc`, edit the plugin source repo, not an installed mirror or this consumer repo); it ships with the `umbraculum-node-react-cursor-assistant` plugin.
