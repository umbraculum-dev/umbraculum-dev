...
- **The `apps/web/app/page.tsx` and `apps/web/app/recipes/**` non-locale routes.** A third anomaly (locale segment skipped) exists at the non-locale root of `apps/web/app/`. Probably legacy; the audit did not investigate. Recorded here so a future audit can pick it up.
