...
**Builds on:** [RFC-0001](../rfcs/0001-modules-tiers-governance-and-automation-placement.md), [RFC-0002](../rfcs/0002-canonical-module-physical-layout.md), [`docs/PLATFORM-ARCHITECTURE.md`](../PLATFORM-ARCHITECTURE.md) §4.4, §6, [`packages/sdk/module-sdk/README.md`](../../packages/sdk/module-sdk/README.md)
...
β layout per [RFC-0002](../rfcs/0002-canonical-module-physical-layout.md): `services/api/src/modules/automation/`, `apps/web/app/[locale]/(automation)/`, `apps/native/src/modules/automation/`, `packages/canonical/automation/contracts/`. Postgres schema `automation` when tables land (§8).
...
registerModule(app, {
  ...
  registerAiTools(registry, app) {
    registerAutomationTools(registry, app.prisma);
  },
...
- New `packages/canonical/automation/contracts/` — mailbox types, adapter SDK types, `CONTRACT_VERSION` constant. No runtime code.
...
| # | Step | Repo | Status |
|---|---|---|---|
| 1 | Scaffold `packages/canonical/automation/contracts/` (types-only, vitest, tsup) | `umbraculum-dev` | **Done** — see commit log |
...
**B-1 invariants captured.** At B-1 ship (2026-05-19), brewery and platform tables were still in `public`; only the new `automation` schema was added. [RFC-0010](../rfcs/0010-platform-brewery-postgres-schema-split.md) (2026-05-28) moved horizontal models to `platform.*` and brewery domain tables to `brewery.*` via forward-only `ALTER SCHEMA` (migration `20260528170000_split_platform_brewery_schemas`). The cross-schema `equipmentProfileId` was L1 app-level at B-1 ship; RFC-0010 upgraded it to a formal Prisma `@relation` to `brewery.EquipmentProfile`. The adapter contract version handshake (`AdapterConnection.contractVersion` ↔ `MAILBOX_SPEC.contractVersion` from `@umbraculum/automation-contracts`) is wired into the table shape but not yet enforced — enforcement lands with the real `brewery.openplc.v1` adapter in Phase C.
