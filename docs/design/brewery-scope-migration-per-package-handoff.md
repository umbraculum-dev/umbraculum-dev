...
- [x] [`packages/platform/test-mcp/package.json`](../../packages/platform/test-mcp/package.json) — update `name` field AND `bin:` field (`brewery-test-mcp` → `umbraculum-test-mcp`).
- [x] [`packages/platform/test-mcp/README.md`](../../packages/platform/test-mcp/README.md) — heading + in-text references + Cursor MCP wiring example's server key (`brewery-test-mcp` → `umbraculum-test-mcp`).
- [x] [`packages/platform/test-mcp/src/server.ts`](../../packages/platform/test-mcp/src/server.ts) — JSDoc comment + `console.log` identity string.

Doc references:
- [x] [`docs/PLATFORM-ARCHITECTURE.md`](../PLATFORM-ARCHITECTURE.md) — §3 inventory mention.
- [x] [`docs/ROLLOUT.md`](../ROLLOUT.md) — 3 references.
- [x] [`docs/TESTING.md`](../TESTING.md) — 3 references (the path reference `packages/platform/test-mcp` stays; only the package-name references change).

Lockfile + dist:
- [ ] Root [`package-lock.json`](../../package-lock.json) — regenerated by `npm install` in container.
- [ ] [`packages/platform/test-mcp/package-lock.json`](../../packages/platform/test-mcp/package-lock.json) — regenerated.
...
- The package ships assets (PNGs, etc.) in `packages/platform/media/assets/`; the *content* is brewery-flavored. The rename does NOT split the content — that's deferred (plan doc §1.4). Do not delete or move assets in this PR.

**File inventory.**

Workspace name + own files:
- [x] [`packages/platform/media/package.json`](../../packages/platform/media/package.json) — update `name` field; add classifying description.
- [x] [`packages/platform/media/README.md`](../../packages/platform/media/README.md) — heading + scope mentions of `@brewery/media` and the `@umbraculum/media` import sample.
...
- [x] [`packages/platform/contracts/README.md`](../../packages/platform/contracts/README.md), [`packages/platform/i18n/README.md`](../../packages/platform/i18n/README.md), [`packages/platform/media/README.md`](../../packages/platform/media/README.md), [`packages/platform/navigation/README.md`](../../packages/platform/navigation/README.md), [`apps/native/README.md`](../../apps/native/README.md), [`apps/web/README.md`](../../apps/web/README.md).
...
Workspace name + own files:
- [x] [`packages/platform/navigation/package.json`](../../packages/platform/navigation/package.json) — `name` field + added classifying description.
- [x] [`packages/platform/navigation/README.md`](../../packages/platform/navigation/README.md) — heading + scope text + both usage code samples (default + `./native` entrypoint) + dependency-stack mention.
- [x] [`packages/platform/navigation/src/native.ts`](../../packages/platform/navigation/src/native.ts) — internal JSDoc reference updated to `@umbraculum/navigation`.
...
Workspace name + own files:
- [x] [`packages/platform/ui/package.json`](../../packages/platform/ui/package.json) — `name` + new classifying `description` (quadruple-entrypoint package; industry-agnostic).
- [x] [`packages/platform/ui/README.md`](../../packages/platform/ui/README.md) — heading + scope/consumed-by paragraphs + npm command samples.
- *(`packages/platform/ui/src/ai/AiChatPanel.tsx` — no `@brewery/ui` self-references; the source imports it had were `@brewery/contracts` + `@brewery/i18n-react`, still pending slots 7+9.)*

Consumer `package.json` deps:
- [x] [`apps/web/package.json`](../../apps/web/package.json).
- [x] [`apps/native/package.json`](../../apps/native/package.json).
- [x] [`packages/verticals/brewery/recipes-ui/package.json`](../../packages/verticals/brewery/recipes-ui/package.json) — recipes-ui itself stays at `@brewery/recipes-ui` for now (slot 13) but its dep was updated.
...
- [x] `packages/verticals/brewery/recipes-ui/src/{mash/MashStepsEditor.tsx, mash/SpargeStepReadOnlyRow.tsx, recipeMeta/RecipeMetaLine.tsx, water/SaltAdditionsEditor.tsx, yeast/ManualCellCountHelpBox.tsx}`.
...
- Classifying `description` added to `packages/verticals/brewery/core/package.json`: brewery-vertical scope explicitly stated + historical "Renamed from @brewery/core to @umbraculum/brewery-core (NOT @umbraculum/core) as sub-plan #9 slot 6" reference preserved verbatim by self-excluding the file from the bulk sed pass.
- Package's own test ([`packages/verticals/brewery/core/src/water.test.js`](../../packages/verticals/brewery/core/src/water.test.js)) updated.
- **NEW HARD STOPS surfaced during slot 6 preflight, not in original inventory:**
  ...
  - `.github/workflows/api.yml` line 43 workflow step display name `Run @brewery/contracts + @brewery/core unit tests` — separate from path globs in `on.push.paths` (which use filesystem paths like `packages/verticals/brewery/core/**` and do NOT need updating).

**File inventory.** All 20 files updated (matches handoff inventory exactly).

Workspace name + own files:
- [x] [`packages/verticals/brewery/core/package.json`](../../packages/verticals/brewery/core/package.json) — `name` → `@umbraculum/brewery-core` (NOT `@umbraculum/core`); classifying description added (excluded from bulk sed to preserve historical reference).
- [x] [`packages/verticals/brewery/core/src/water.test.js`](../../packages/verticals/brewery/core/src/water.test.js) — test imports.

Consumer `package.json` deps:
- [x] [`apps/web/package.json`](../../apps/web/package.json).
- [x] [`services/api/package.json`](../../services/api/package.json).
- [x] [`packages/verticals/brewery/beerjson/package.json`](../../packages/verticals/brewery/beerjson/package.json).
...
- [x] [`packages/verticals/brewery/beerjson/src/index.ts`](../../packages/verticals/brewery/beerjson/src/index.ts).
...
Cross-package README references:
- [x] [`packages/verticals/brewery/beerjson/README.md`](../../packages/verticals/brewery/beerjson/README.md), [`apps/web/README.md`](../../apps/web/README.md), [`services/api/README.md`](../../services/api/README.md).
...
- `i18n-react` depends on `i18n`. Originally predicted as "After this slot ships, `i18n-react` still imports `@brewery/i18n` — transient cross-scope state until slot 8". **Actual post-rename state:** the bulk sweep properly updated `packages/platform/i18n-react/package.json` deps AND `packages/platform/i18n-react/src/index.tsx` imports to `@umbraculum/i18n`; the rebuilt `packages/platform/i18n-react/dist/*` artifacts reference `@umbraculum/i18n` only (verified `grep -rohE "@(brewery|umbraculum)/i18n" packages/platform/i18n-react/dist/` → `@umbraculum/i18n` only). The transient cross-scope state is only that the i18n-react **workspace itself is still named** `@brewery/i18n-react` — its CONTENT (deps + imports + dist) is fully migrated to the new scope. Slot 8 will rename the workspace name itself.
- The package ships a JSON locale bundle and a `copy-json.mjs` script ([`packages/platform/i18n/scripts/`](../../packages/platform/i18n/scripts/)). The script copies content into `dist/` and was re-run successfully via `npm run build:packages` in step 5. Verified `packages/platform/i18n/dist/` contains `en.json`, `it.json`, `index.cjs`, `index.d.cts`, `index.d.ts`, `index.js`.

**File inventory (all checked Done 2026-05-19).**

Workspace name + own files:
- [x] [`packages/platform/i18n/package.json`](../../packages/platform/i18n/package.json) — `name` → `@umbraculum/i18n`; classifying description added (platform-framework; brewery-flavored content per plan doc §1.4 deferred-content-split).
- [x] [`packages/platform/i18n/README.md`](../../packages/platform/i18n/README.md) — heading + brand callout updated.

Consumer `package.json` deps:
- [x] [`apps/web/package.json`](../../apps/web/package.json).
- [x] [`apps/native/package.json`](../../apps/native/package.json).
- [x] [`packages/platform/i18n-react/package.json`](../../packages/platform/i18n-react/package.json) — i18n-react workspace still NAMED `@brewery/i18n-react` (slot 8 surface) but its `dependencies` entry for the i18n peer is now `@umbraculum/i18n`.
...
- [x] [`apps/web/src/navigation/appRouter.ts`](../../apps/web/src/navigation/appRouter.ts).
- [x] [`packages/platform/i18n-react/src/index.tsx`](../../packages/platform/i18n-react/src/index.tsx) — imports updated to `@umbraculum/i18n`; dist rebuilt and verified to reference `@umbraculum/i18n` only.

Cross-package README references:
- [x] [`packages/platform/contracts/README.md`](../../packages/platform/contracts/README.md), [`packages/platform/i18n-react/README.md`](../../packages/platform/i18n-react/README.md), [`packages/platform/i18n/README.md`](../../packages/platform/i18n/README.md), [`packages/platform/media/README.md`](../../packages/platform/media/README.md), [`packages/platform/navigation/README.md`](../../packages/platform/navigation/README.md), [`apps/native/README.md`](../../apps/native/README.md), [`apps/web/README.md`](../../apps/web/README.md).
...
- [x] No `bin` field in [`packages/platform/i18n/package.json`](../../packages/platform/i18n/package.json) (preflight command 2 PASS — nothing to update in root `node_modules/.bin/` symlinks).
...
- Predecessor: slot 7 (`@brewery/i18n` → `@umbraculum/i18n`) shipped 2026-05-19 — confirmed in slot 8 preflight via `grep '"@umbraculum/i18n"' packages/platform/i18n-react/package.json` returning the pre-existing dep entry (slot 7 had already updated it during the transient-cross-scope handling).
- Two-entry-point shape: `./` default + `./next-intl` Next.js variant — both `dist/{index,next-intl}.{js,cjs,d.ts,d.cts}` must rebuild. Verified post-step-5: all 8 expected artifacts present in `packages/platform/i18n-react/dist/`.

**File inventory (all checked Done 2026-05-19).**

Workspace name + own files:
- [x] [`packages/platform/i18n-react/package.json`](../../packages/platform/i18n-react/package.json) — `name` → `@umbraculum/i18n-react`; classifying description added (platform; universal hook; no brewery-domain logic).
- [x] [`packages/platform/i18n-react/README.md`](../../packages/platform/i18n-react/README.md) — heading + brand callout + dual-entry-point examples + cross-link to `@brewery/recipes-ui` (with slot 13 forward-ref) updated.
- [x] [`packages/platform/i18n-react/src/index.tsx`](../../packages/platform/i18n-react/src/index.tsx) — no internal `@brewery/i18n-react` references found in source (the package doesn't self-import; was already clean).

Consumer `package.json` deps:
- [x] [`apps/web/package.json`](../../apps/web/package.json).
- [x] [`apps/native/package.json`](../../apps/native/package.json).
- [x] [`packages/verticals/brewery/recipes-ui/package.json`](../../packages/verticals/brewery/recipes-ui/package.json).
...
- [x] [`packages/verticals/brewery/recipes-ui/src/recipeMeta/RecipeMetaLine.tsx`](../../packages/verticals/brewery/recipes-ui/src/recipeMeta/RecipeMetaLine.tsx).
- [x] [`packages/verticals/brewery/recipes-ui/src/water/SaltAdditionsEditor.tsx`](../../packages/verticals/brewery/recipes-ui/src/water/SaltAdditionsEditor.tsx).
- [x] [`packages/verticals/brewery/recipes-ui/src/yeast/ManualCellCountHelpBox.tsx`](../../packages/verticals/brewery/recipes-ui/src/yeast/ManualCellCountHelpBox.tsx).
- [x] [`packages/platform/ui/src/ai/AiChatPanel.tsx`](../../packages/platform/ui/src/ai/AiChatPanel.tsx).

Cross-package README references:
- [x] [`packages/platform/i18n-react/README.md`](../../packages/platform/i18n-react/README.md), [`packages/platform/i18n/README.md`](../../packages/platform/i18n/README.md), [`packages/verticals/brewery/recipes-ui/README.md`](../../packages/verticals/brewery/recipes-ui/README.md), [`apps/native/README.md`](../../apps/native/README.md), [`apps/web/README.md`](../../apps/web/README.md).
...
- [x] No `bin` field in [`packages/platform/i18n-react/package.json`](../../packages/platform/i18n-react/package.json) (preflight command 2 PASS — nothing to update in root `node_modules/.bin/` symlinks).
...
- [x] [`packages/platform/contracts/package.json`](../../packages/platform/contracts/package.json) — `name: "@umbraculum/contracts"` + new classifying `description` field.
- [x] [`packages/platform/contracts/README.md`](../../packages/platform/contracts/README.md) — heading + scoped workspace commands updated; bare brand callout rewritten to record the rename history (slot 9 of 14; first slot to open two simultaneous transient cross-scope states).
- [x] [`packages/platform/contracts/src/ai/aiTool.ts`](../../packages/platform/contracts/src/ai/aiTool.ts) — internal `@brewery/contracts` reference + forward-looking `@brewery/ai-tool-sdk` reference both updated to `@umbraculum/*` scope.
...
- [x] [`packages/platform/api-client/README.md`](../../packages/platform/api-client/README.md), [`packages/platform/i18n/README.md`](../../packages/platform/i18n/README.md), [`packages/platform/media/README.md`](../../packages/platform/media/README.md), [`packages/platform/navigation/README.md`](../../packages/platform/navigation/README.md), [`packages/verticals/brewery/recipes-ui/README.md`](../../packages/verticals/brewery/recipes-ui/README.md), [`apps/web/README.md`](../../apps/web/README.md), [`apps/native/README.md`](../../apps/native/README.md), [`services/api/README.md`](../../services/api/README.md). (Own README excluded per item 11.)
...
- [x] Predecessor: slot 9 shipped — confirmed `packages/platform/api-client/package.json` already had `"@umbraculum/contracts": "file:../contracts"` at start of slot 10 (slot-9 sweep had updated the dep entry; only the workspace `name` field remained to flip).
...
Workspace name + own files:
- [x] [`packages/platform/api-client/package.json`](../../packages/platform/api-client/package.json) — `name` flipped; `description` added (slot-8 precedent); the slot-9 sweep had already updated the `@umbraculum/contracts` dep entry, so no further dep changes needed.
- [x] [`packages/platform/api-client/README.md`](../../packages/platform/api-client/README.md) — heading + workspace command examples updated; rename history note added per slot-8 / slot-9 convention.
...
Cross-package README references:
- [x] [`packages/platform/api-client/README.md`](../../packages/platform/api-client/README.md), [`packages/platform/contracts/README.md`](../../packages/platform/contracts/README.md), [`apps/native/README.md`](../../apps/native/README.md), [`apps/web/README.md`](../../apps/web/README.md), [`services/api/README.md`](../../services/api/README.md).
...
**Step-3 sweep results:** 31 files / 41 substitutions (vs 35-file plan-doc inventory; the 4-file delta = 1 self-exclusion of `packages/platform/api-client/package.json` because step 1 had already updated the `name` field + 3 explicit excludes — own README + 2 history docs).
...
- [x] Predecessor: slot 6 shipped — confirmed `packages/verticals/brewery/beerjson/package.json` already lists `"@umbraculum/brewery-core": "file:../core"` as its dep; only the workspace `name` field flipped in slot 12.
...
Workspace name + own files:
- [x] [`packages/verticals/brewery/beerjson/package.json`](../../packages/verticals/brewery/beerjson/package.json) — `name` flipped to `@umbraculum/brewery-beerjson`; `description` field added per handoff (reworded from forecast to current-state).
- [x] [`packages/verticals/brewery/beerjson/README.md`](../../packages/verticals/brewery/beerjson/README.md) — heading + workspace command examples + Usage import example updated; rename history note added per slot-8/9/10/11 convention with explicit TRAP rationale (the `brewery-` prefix marks brewery-vertical classification, not the platform-name `@umbraculum/beerjson`).
- [x] [`packages/verticals/brewery/beerjson/src/index.ts`](../../packages/verticals/brewery/beerjson/src/index.ts) — already clean (the slot-6 sweep had updated the `@umbraculum/brewery-core` import; no `@brewery/beerjson` self-references in this file).

Consumer `package.json` deps:
- [x] [`apps/web/package.json`](../../apps/web/package.json).
- [x] [`apps/native/package.json`](../../apps/native/package.json).
- [x] [`packages/verticals/brewery/recipes-ui/package.json`](../../packages/verticals/brewery/recipes-ui/package.json) — recipes-ui dep.
...
- [x] [`packages/verticals/brewery/recipes-ui/src/mash/MashStepsEditor.tsx`](../../packages/verticals/brewery/recipes-ui/src/mash/MashStepsEditor.tsx) — 2 substitutions.

Cross-package README references:
- [x] [`packages/verticals/brewery/beerjson/README.md`](../../packages/verticals/brewery/beerjson/README.md), [`apps/native/README.md`](../../apps/native/README.md), [`apps/web/README.md`](../../apps/web/README.md).
...
**Step-3 sweep results:** 21 files / 29 substitutions (vs 24-file plan-doc inventory; the 3-file delta = 3 explicit excludes — own README + 2 history docs; `packages/verticals/brewery/beerjson/package.json` self-excluded after step 1 had already updated the `name` field).
...
- [x] Predecessors: slots 5 (`ui`), 8 (`i18n-react`), 12 (`beerjson`) all shipped — confirmed `packages/verticals/brewery/recipes-ui/package.json` lists `@umbraculum/brewery-beerjson`, `@umbraculum/i18n-react`, `@umbraculum/ui` as deps before step 1.
- [x] **`apps/native/metro.config.js`** line 31 `extraNodeModules: { "@brewery/recipes-ui": ... }` updated to `@umbraculum/brewery-recipes-ui` by the bulk sweep — the on-disk symlink target stays as `packages/verticals/brewery/recipes-ui` (the directory name doesn't change). This is the only metro `extraNodeModules` pin in the repo and slot-13's flagship HARD STOP. Cleared by sweep.
...
Workspace name + own files:
- [x] [`packages/verticals/brewery/recipes-ui/package.json`](../../packages/verticals/brewery/recipes-ui/package.json) — `name` flipped to `@umbraculum/brewery-recipes-ui`; `description` field added per handoff (reworded from forecast to current-state).
- [x] [`packages/verticals/brewery/recipes-ui/README.md`](../../packages/verticals/brewery/recipes-ui/README.md) — heading + workspace command examples + future-package-name examples updated; rename history note added per slot-8/9/10/11/12 convention with explicit TRAP rationale (`brewery-` prefix marks brewery-vertical classification, not the platform-name `@umbraculum/recipes-ui` — the latter is the explicit anti-pattern in `docs/modules/contribute/horizontal-package.md:113`).
- [x] All internal `packages/verticals/brewery/recipes-ui/src/**/*.tsx` files — already on `@umbraculum/brewery-beerjson`, `@umbraculum/i18n-react`, `@umbraculum/ui` from prior slots; no in-package self-reference to update (recipes-ui doesn't import itself).
...
- [x] [`packages/verticals/brewery/beerjson/README.md`](../../packages/verticals/brewery/beerjson/README.md) — 4 substitutions, [`packages/platform/contracts/README.md`](../../packages/platform/contracts/README.md), [`packages/platform/i18n-react/README.md`](../../packages/platform/i18n-react/README.md) — 2 substitutions + 2 tautology purges, [`packages/platform/media/README.md`](../../packages/platform/media/README.md) — 1 substitution + 1 tautology purge, [`packages/platform/navigation/README.md`](../../packages/platform/navigation/README.md) — 2 substitutions + 2 tautology purges, [`packages/platform/ui/README.md`](../../packages/platform/ui/README.md) — 4 substitutions + 4 tautology purges, [`apps/native/README.md`](../../apps/native/README.md) — 4 substitutions, [`apps/web/README.md`](../../apps/web/README.md) — 4 substitutions.
...
- [x] Plugin-pack skill `package-scope-migration-preflight` published at `<umbraculum-toolset>/cursor-plugins/umbraculum-platform-tsjs-cursor-assistant/skills/package-scope-migration-preflight/SKILL.md` and referenced from the plan doc §4 step 2; refined through slots 6 → 8 (4-cousin walk added) → 13 (forecast-becomes-live tautology-purge step added) → 14 (operational closing-condition note to be added in the next plugin-pack revision). *(Historical path note: pre-2026-06-11 docs cited `~/.cursor/plugins/local/`.)*
