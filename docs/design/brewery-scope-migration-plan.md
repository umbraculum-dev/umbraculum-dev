...
| Slots landed | 9 of 14 — slots 1–9 (`test-mcp`, `media`, `navigation`, `automation-contracts`, `ui`, `brewery-core`, `i18n`, `i18n-react`, `contracts`). **Slot 9 opens TWO simultaneous transient cross-scope states** (slot 10 `api-client` + slot 11 `module-sdk` workspaces still on `@brewery/*` while their imports + dist already point at `@umbraculum/contracts`); both close on their respective slots. First slot to validate the multi-state version of the upstream-then-downstream-rename ordering. |
...
| Remaining slots to migrate | 5 (4 packages + 1 application-workspace bundle); see [handoff doc](./brewery-scope-migration-per-package-handoff.md) |
| Estimated remaining sessions | 2–3 (slots 10 + 11 together likely close the two transient cross-scope states from slot 9; slots 12 + 13 + 14 batchable per cadence) |
