...
**Tier:** Public
**Status:** Active 2026-05-19 — scoping pass + slots 1–12 landed (slot 1: `test-mcp` worked example, §6; slot 2: `media`, §6.1; slot 3: `navigation`, §6.2; slot 4: `automation-contracts`, §6.3; slot 5: `ui` — heavy 69-file slot held cleanly on first attempt under the slot-4-corrected recipe, §6.4; slot 6: `core` → `brewery-core` — ⚠ TRAP slot resolved cleanly on first attempt under the §1.3 classification gate, §6.5; slot 7: `i18n` — first slot with controlled transient cross-scope state (i18n-react workspace was still named `@brewery/i18n-react` at end of slot 7 but its imports + dist were fully on `@umbraculum/i18n`), §6.6; **post-slot-7 CI hygiene fix #2 isolated three independent local-vs-CI divergence root causes** — gitignored cross-references, nested-workspace install drift, stale bind-mounted `node_modules` shadowing, §6.7 + new `scripts/ci-parity-check.sh` + new umbraculum-toolset rule 72 + new `ci-parity-local-reproduction` skill; slot 8: `i18n-react` — completed the i18n stack migration started in slot 7; closed the transient cross-scope state via the standard recipe; native typecheck via the no-root-install pattern (named-volume mount only) successfully averted the slot-5 GOTCHA, §6.8; slot 9: `contracts` — heaviest slot at 75 actually-edited files / 124 substitutions; first slot to open TWO simultaneous transient cross-scope states (slots 10 + 11 both depend on `contracts`); recipe held cleanly on first attempt; surfaced two new lessons (`cursor-tmp/` bulk-sed self-corruption, mixed-scope shorthand-comment slop) folded into §4 step 3 + §5, §6.9; slot 10: `api-client` — closed one of slot-9's two transient states (the `@brewery/api-client` half); recipe held cleanly on first attempt with no new lessons; first slot whose preflight identified only ONE HARD STOP class (root `build:packages`) — narrowing per-slot blast radius confirms migration is approaching the tail-end, §6.10; slot 11: `module-sdk` — closed the LAST slot-9 transient state (the `@brewery/module-sdk` half); the multi-state version of upstream-then-downstream-rename ordering held cleanly in practice across slots 9→{10,11} with no observed inter-state interaction; F5 row in `docs/CONTRACTS-VALIDATION-STRATEGY.md` marked Done; api `registerModule()` hot path verified clean post-rename; recipe held with no new lessons, §6.11; slot 12: `beerjson` → `brewery-beerjson` — first brewery-vertical slot since slot 6; slot-6 TRAP discipline (target carries `brewery-` prefix) held cleanly on first attempt across 29 substitutions; recipe held with no new lessons, §6.12; each slot surfaced lessons folded back into §4/§5 before commit). Remaining 2 slots (1 brewery-vertical package + 1 application-workspace bundle) tracked in [`brewery-scope-migration-per-package-handoff.md`](./brewery-scope-migration-per-package-handoff.md).
...
| Slots landed | 12 of 14 — slots 1–12 (`test-mcp`, `media`, `navigation`, `automation-contracts`, `ui`, `brewery-core`, `i18n`, `i18n-react`, `contracts`, `api-client`, `module-sdk`, `brewery-beerjson`). Slot 12 was the first brewery-vertical slot since slot 6's TRAP-resolution; the slot-6 TRAP discipline (target carries `brewery-` prefix in the package name) held cleanly across slot 12's 29 substitutions on first attempt. |
...
| Remaining slots to migrate | 2 (1 brewery-vertical package + 1 application-workspace bundle); see [handoff doc](./brewery-scope-migration-per-package-handoff.md) |
| Estimated remaining sessions | 1 (slot 13 + slot 14 likely batchable in a single session — slot 13 closes the last brewery-vertical package; slot 14 is the four application-workspace `name` flips with zero workspace consumers) |
...
**Commit hash:** `c468bf6` (amended from `bd74442` to add the missing `RENAME-DILIGENCE.md` link in the module-sdk README NOTE block — exact slot-8 lesson replay caught pre-push by `ci-parity-check.sh`'s docs-readmes job).
