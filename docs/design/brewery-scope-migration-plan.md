...
| Slots landed | 10 of 14 — slots 1–10 (`test-mcp`, `media`, `navigation`, `automation-contracts`, `ui`, `brewery-core`, `i18n`, `i18n-react`, `contracts`, `api-client`). **Slot 10 closes one of slot-9's two simultaneous transient cross-scope states** (`@brewery/api-client` is now `@umbraculum/api-client`); the `@brewery/module-sdk` half remains open until slot 11. The multi-state version of upstream-then-downstream-rename ordering held in practice — no observed interaction between the two open states during slot 10's execution. |
...
| Remaining slots to migrate | 4 (3 packages + 1 application-workspace bundle); see [handoff doc](./brewery-scope-migration-per-package-handoff.md) |
| Estimated remaining sessions | 2 (slot 11 closes the last slot-9 transient state; slots 12 + 13 + 14 batchable per cadence) |
