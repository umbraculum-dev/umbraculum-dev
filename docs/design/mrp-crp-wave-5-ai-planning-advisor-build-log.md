...
| Phase A — contract and tool-surface audit | Completed | `packages/modules/mrp-contracts/src/aiTools.test.ts`, `packages/modules/crp-contracts/src/aiTools.ts`, `packages/modules/crp-contracts/src/aiTools.test.ts`, `packages/modules/crp-contracts/src/index.ts` | Pending Phase F gates | MRP's planned schemas already matched Wave 5. CRP still exposed future schedule aggregate schemas, so Wave 5 adds generic read schemas for the shipped work-center, scheduled-operation, capacity-load, and conflict surfaces without inventing schedule rows. |
...
- `docker run --rm -v "$PWD/packages/platform/rendering:/repo/packages/platform/rendering" -w /repo/packages/platform/rendering node:20-slim npm install --no-audit --no-fund --no-package-lock` — runtime repair only; restored MJML transitive dependencies needed by API test imports.
