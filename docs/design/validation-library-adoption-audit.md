...
- **The plugin-pack rule `.cursor/rules/22-typescript-contracts-runtime-validation.mdc`** in its current form ("boundary payloads must be parsed not cast; do NOT introduce Zod / Valibot / TypeBox just because a parser is being edited") gets rewritten to enforce the new pattern.
