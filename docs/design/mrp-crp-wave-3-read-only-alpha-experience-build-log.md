...
| Phase A — route ownership, navigation, i18n | Completed | `packages/platform/navigation/src/index.ts`, `packages/platform/navigation/src/native.ts`, `apps/web/app/_components/PrimaryNav.tsx`, `scripts/check-web-url-segments.ts`, `packages/platform/i18n/src/en.json`, `packages/platform/i18n/src/it.json`, `apps/web/package.json`, lockfiles, `docker-compose.yml` web mounts | Focused E2E verifies localized nav/page labels; route-segment gate passed | MRP/CRP use their already-registered owned URL segments. Native route availability remains blocked. |
...
- `docker compose exec -T web npm run typecheck` — exits non-zero on the existing Tamagui shorthand baseline outside Wave 3; the final rerun had no matches for Wave 3 files, `PrimaryNav`, `packages/platform/navigation`, or `scripts/check-web-url-segments.ts`.
- `docker run --rm -v "$PWD:/repo" -w /repo/packages/platform/navigation node:20-slim bash -lc "npm run typecheck"` — passed.
- `docker run --rm -v "$PWD:/repo" -w /repo node:20-slim bash -lc "npx eslint apps/web/app/'[locale]'/'(mrp)' apps/web/app/'[locale]'/'(crp)' apps/web/e2e/smoke/mrp-crp-read-only-alpha.spec.ts apps/web/app/_components/PrimaryNav.tsx packages/platform/navigation/src/index.ts packages/platform/navigation/src/native.ts scripts/check-web-url-segments.ts --max-warnings 0"` — passed.
...
- API health was restored to HTTP 200 after the package rebuild by restarting the API and applying the documented `packages/platform/rendering` dependency bootstrap for the known rendering package realpath resolution gap.
