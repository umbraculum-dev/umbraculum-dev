...
| Phase A — contracts | Shipped | `packages/canonical/mrp/contracts`, `packages/canonical/crp/contracts` `documentTemplates.ts` + dist | `npm run test -w @umbraculum/mrp-contracts` / `crp-contracts` in container |
