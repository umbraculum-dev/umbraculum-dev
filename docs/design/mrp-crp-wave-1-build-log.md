...
- `types-baseline-verifier` — OK for `packages/canonical/mrp/contracts`, `packages/canonical/crp/contracts`, and `services/api`.
...
- The API container needed to be recreated so the new `packages/canonical/mrp/contracts` and `packages/canonical/crp/contracts` bind mounts were visible.
