...
var RenderKindSchema = import_zod2.z.enum([
  "pdf",
  "xlsx",
  "csv",
  "docx",
...
var RenderStatusSchema = import_zod2.z.enum([
  "queued",
  "running",
  "succeeded",
  "failed"
]);
var RenderVisibilitySchema = import_zod2.z.enum(["workspace", "public"]);
var RenderDeliverySchema = import_zod2.z.discriminatedUnion("mode", [
  import_zod2.z.object({ mode: import_zod2.z.literal("stream-response") }).strict(),
  import_zod2.z.object({
    mode: import_zod2.z.literal("persist-to-media"),
    visibility: RenderVisibilitySchema
  }).strict(),
  import_zod2.z.object({
    mode: import_zod2.z.literal("email"),
    to: import_zod2.z.array(import_zod2.z.string().email()).min(1, "email.to required"),
    subject: import_zod2.z.string().min(1, "email.subject required")
  }).strict()
]);
var RenderErrorSchema = import_zod2.z.object({
  code: import_zod2.z.string().min(1, "error.code required"),
  message: import_zod2.z.string().min(1, "error.message required")
}).strict();
var RenderJobSubmitRequestSchema = import_zod2.z.object({
  templateRef: import_zod2.z.string().min(1, "templateRef required"),
  kind: RenderKindSchema.optional(),
  data: import_zod2.z.unknown(),
  delivery: RenderDeliverySchema.optional()
}).strict();
var RenderJobStatusSchema = import_zod2.z.object({
  id: import_zod2.z.string().min(1, "job.id required"),
  templateRef: import_zod2.z.string().min(1, "job.templateRef required"),
  kind: RenderKindSchema,
  status: RenderStatusSchema,
  deliveryMode: import_zod2.z.string().min(1, "job.deliveryMode required"),
  requestedAt: import_zod2.z.string().min(1, "job.requestedAt required"),
  startedAt: import_zod2.z.string().nullable(),
  completedAt: import_zod2.z.string().nullable(),
  artifactId: import_zod2.z.string().nullable(),
  mediaAssetId: import_zod2.z.string().nullable(),
  error: RenderErrorSchema.nullable()
}).strict();
var RenderJobSubmitResponseSchema = import_zod2.z.object({
  ok: import_zod2.z.literal(true),
  mode: import_zod2.z.literal("async"),
  job: RenderJobStatusSchema
}).strict();
var RenderJobStatusResponseSchema = import_zod2.z.object({
  ok: import_zod2.z.literal(true),
  job: RenderJobStatusSchema
}).strict();
var RenderJobCancelResponseSchema = import_zod2.z.object({
  ok: import_zod2.z.literal(true),
  job: RenderJobStatusSchema
}).strict();
var RenderJobResultResponseSchema = import_zod2.z.object({
  ok: import_zod2.z.literal(true),
  job: RenderJobStatusSchema,
  signedUrl: import_zod2.z.string().min(1, "signedUrl required"),
  expiresAt: import_zod2.z.string().min(1, "expiresAt required")
}).strict();
