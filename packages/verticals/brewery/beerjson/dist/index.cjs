...
__export(index_exports, {
  ...
  sgToPlato: () => import_brewery_core3.sgToPlato,
  validateMashBeforeSave: () => validateMashBeforeSave
});
module.exports = __toCommonJS(index_exports);
var import_brewery_core3 = require("@umbraculum/brewery-core");

// src/beerJsonPrimitives.ts
...
// src/beerJsonIngredientBuilders.ts
...
// src/beerJsonParseGrist.ts
function parseGristRowsFromBeerJsonIngredients(ing) {
  const fermentables = Array.isArray(ing["fermentable_additions"]) ? ing["fermentable_additions"] : [];
  return fermentables.map((fUnknown) => {
...
// src/beerJsonParseHops.ts
var VALID_HOP_FORMS = ["extract", "leaf", "leaf (wet)", "pellet", "powder", "plug"];
function parseHopRowsFromBeerJsonIngredients(ing) {
  const hops = Array.isArray(ing["hop_additions"]) ? ing["hop_additions"] : [];
  return hops.map((hUnknown) => {
...
// src/beerJsonParseMash.ts
function parseMashFromBeerJsonRecipeRoot(r0) {
  return parseMashFromBeerJson(r0);
}

// src/beerJsonParseMisc.ts
var VALID_MISC_TYPES = ["spice", "fining", "water_agent", "herb", "flavor", "other"];
function parseMiscRowsFromBeerJsonIngredients(ing) {
  const misc = Array.isArray(ing["miscellaneous_additions"]) ? ing["miscellaneous_additions"] : [];
  return misc.map((mUnknown) => {
...
// src/beerJsonParseYeast.ts
function parseYeastRowsFromBeerJsonIngredients(ing) {
  const cultures = Array.isArray(ing["culture_additions"]) ? ing["culture_additions"] : [];
  return cultures.map((cUnknown) => {
...
var import_brewery_core2 = require("@umbraculum/brewery-core");
function computeEstimatedCellsB(batchSizeLiters, ogEstimatedSg, pitchRateKey) {
  ...
  if (typeof ogEstimatedSg !== "number" || !Number.isFinite(ogEstimatedSg) || ogEstimatedSg <= 1)
    return null;
  const plato = (0, import_brewery_core2.sgToPlato)(ogEstimatedSg);
